...
After a key closes, `answers` steers the owning task through `bin/fm-send.sh` (the recorded `Origin:` task, else the legacy `<origin>-decision-<key>` prefix, else the held task itself, whichever has live metadata), deduped by a `steer-<digest>` token in the inbox record; a channel that already delivered the answer itself passes `--no-steer`.
