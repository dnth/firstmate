...
A task whose metadata records no owner token, including every persistent-TUI launch from before the token existed, or whose profile registry file is gone, warns and reaps only the worktree-rooted processes, exactly as teardown behaved before the token existed.
