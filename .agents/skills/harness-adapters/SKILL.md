...
The default OMP launch message is the encoded full brief body (`fm-operational-input.sh encode launch-brief` emits `⁣FIRSTMATE_OP: v1 launch-brief: <body>` as the positional argument). File reads mid-session are tool results and are never scanned for keywords.
...
For an opted-in OMP launch, `bin/fm-spawn.sh` passes two positional user messages: the standalone `orchestrate` keyword, followed by the unchanged encoded brief. For an ordinary (non-opted) OMP launch it passes only the unchanged encoded brief.
