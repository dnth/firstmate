...
`bin/fm-spawn.sh` accepts concrete `--harness`, `--model`, and `--effort` values chosen by firstmate at intake, plus `--prewalk-into` when an OMP profile explicitly carries `prewalk_into`.
Pass that OMP target unchanged; [`docs/configuration.md`](../../../docs/configuration.md#crew-dispatch-profiles-configcrew-dispatchjson) owns the schema and fallback semantics.
...
`fm-send` handles it the same way it handles `/` - it gives the popup a longer settle (1.2s) between typing and the first Enter, with the target backend's submit retry as the safety net - but the `$` settle is scoped to `harness=codex`, read from target metadata for task selectors or explicit endpoints that match a recorded task.
That scope matters because, unlike `/`, a leading `$` commonly starts ordinary text (`$5/month`, `$HOME`); only a Codex target treats `$...` as a typed skill invocation, while ordinary local `$...` text for every other harness follows the durable inbox plane.
An unmatched explicit `session:window` target has no metadata, so its harness is unknown and treated as non-Codex (the safe fast-path default); an explicit endpoint that matches task metadata stays on the typed plane but retains that recorded harness context.
...
**Busy-queued Enter (opencode 1.18.4 and OMP).**
...
The default OMP launch message is the encoded full brief (`fm-operational-input.sh encode launch-brief` reads the brief body on stdin and emits `⁣FIRSTMATE_OP: v1 launch-brief: <body>` as the positional argument), so standalone `orchestrate` in a brief's own prose reaches the matcher even without the opt-in - brief authors must write `orchestration` or backtick `orchestrate` to discuss the mode without triggering it; file reads mid-session are tool results and are never scanned.
`bin/fm-brief.sh --orchestrate` records the opt-in for an ordinary ship task as the non-matching marker `orchestration: enabled`; `bin/fm-spawn.sh` then launches the OMP worker with an initial message containing the standalone keyword plus the brief pointer, so activation is an explicit firstmate-authored signal rather than ambiguous brief prose.
...
Secondmate spawns skip the pointer because parent supervision uses secondmate-home watcher liveness rather than Grok turn-end markers.
