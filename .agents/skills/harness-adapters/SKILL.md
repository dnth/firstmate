...
The adapter stays inert inside an fm-spawn task worker's launch environment (`FM_TASK_ID`, stamped on every non-secondmate launch), so a project-extension-loaded copy in a worker's worktree never activates primary integration even when the worktree still carries a retired secondmate's `.fm-secondmate-home` marker.
