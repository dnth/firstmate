...
The adapter requires both executables to be explicitly listed in `FM_WHEN_ALLOWLIST` (with only `true` and `false` allowed by default), and rejects byte-identical copies of this fork's guarded entrypoints regardless of allowlist location.
Only allowlist entries whose provenance you have reviewed are eligible, so never list a wrapper that reaches a refused entrypoint or bypasses its existing consent, confirmation, or lifecycle gate.
