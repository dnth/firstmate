...
The adapter fail-closes obvious bypasses by refusing generic command carriers, common destructive or remote-control commands, and this fork's RunPod, X-mode, and public-followup entrypoints as either half of a watch, and it byte-binds both direct executables.
...
RunPod provision, wake, sleep, recovery, or destruction and every live X-mode or promised-public-reply mutation always stay on those existing guarded flows; a condition may inspect their state through a purpose-built read-only command but may not invoke their lifecycle or reply entrypoints.
