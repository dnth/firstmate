...
The adapter fail-closes obvious bypasses by refusing generic command carriers, common destructive or remote-control commands, and this fork's RunPod and remote lifecycle, PR merge and teardown, X-mode, and public-followup entrypoints as either half of a watch, and it byte-binds both direct executables.
That mechanical boundary does not make an arbitrary custom executable safe or inspect what it may call, so never use a wrapper to reach a refused entrypoint or to bypass its existing consent, confirmation, or lifecycle gate.
