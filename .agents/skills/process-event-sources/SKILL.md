...
description: >-
  ...
  durable result read, the handled acknowledgement contract, the one-owner rule,
  the precise durability boundary, and the Lavish adapter's loss limitation.
...
```sh
bin/fm-captain-hold.sh bind <source-id> [<origin-id>]
```
...
Eligibility is a firstmate judgment made BEFORE arming: both commands must be safe, reversible, and exact (for example `no-mistakes update --beta`, whose own guard refuses while a validation run is active).
The adapter requires both executables to be explicitly listed in `FM_WHEN_ALLOWLIST` (with only `true` and `false` allowed by default).
Eligibility is based on the registered executable path and bytes, and does not inspect commands an allowlisted executable invokes internally.
...
RunPod provision, wake, sleep, recovery, or destruction, remote lifecycle control, PR merge and worktree teardown, and every live X-mode or promised-public-reply mutation always stay on those existing guarded flows; a condition may inspect their state through a purpose-built read-only command but may not invoke their lifecycle or reply entrypoints.
...
: Ask the adapter what the result means rather than parsing it yourself.
  `bin/fm-procevent-lavish.sh classify <result-file>` returns `feedback`, `ended`, `waiting`, `missing`, or `unknown`.
  Consume a Lavish capture with `bin/fm-procevent-lavish.sh read <result-file>` rather than grepping the raw file: it enumerates every item, reports completeness, and keeps the session-ending message distinct from annotations.
