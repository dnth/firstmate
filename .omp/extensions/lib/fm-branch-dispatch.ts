...
export interface UnreadWakeScope {
   ...
   * branch to drain and acknowledge right now (docs/omp-supervision-branch.md
   ...
   * declaration is captain-held. fm-primary-omp.ts cross-references these
...
const EMPTY_SCOPE: UnreadWakeScope = { status: "empty", eligible: false, projects: [], eligibleSeqs: [], eligibleTasks: [], corrupted: false, needsDecisionKeys: [], taskByWakeKey: {} };
const UNSAFE_SCOPE: UnreadWakeScope = { status: "unsafe", eligible: false, projects: [], eligibleSeqs: [], eligibleTasks: [], corrupted: true, needsDecisionKeys: [], taskByWakeKey: {} };
...
// (.omp/extensions/fm-primary-omp.ts forces every check-kind TRIGGER to main), so nothing
...
// and forced to main on its own triggering close (fm-primary-omp.ts's
...
// no longer rides the heartbeat into main (docs/omp-supervision-branch.md
// "Heartbeat routing").
//
