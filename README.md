...
- **OMP harness with a read-only credential broker** - OMP runs as a verified crew, scout, and primary runtime, while a remote pod holds only a restricted, read-only broker bearer in a mode-600 file - never in the RunPod environment, command arguments, or logs - and can reach only the loopback facade ([bin/fm-omp-auth-broker-readonly-proxy.mjs](bin/fm-omp-auth-broker-readonly-proxy.mjs)) over its SSH reverse tunnel; the facade rejects every credential-mutation endpoint, so the pod receives only redacted credential snapshots with refreshes performed server-side and never the workstation's Claude or GPT subscription login or refresh credentials; see [docs/supervision-protocols/omp.md](docs/supervision-protocols/omp.md) for the OMP watcher protocol.
