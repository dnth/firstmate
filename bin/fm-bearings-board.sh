...
# Write each captains_call card of the compacted payload JSON to
# $STATE/decision-cards/<key>.json as an fm-decision-card.v1 record, atomically.
# Keys outside the task-id slug shape are skipped rather than trusted as paths.
write_decision_cards() {  # <compacted-payload-json>
  local payload=$1 dir="$STATE/decision-cards" card_key card_tmp
  (umask 077; mkdir -p "$dir") || fail "cannot create $dir"
  while IFS= read -r card_key; do
    [ -n "$card_key" ] || continue
    case "$card_key" in ''|*[!A-Za-z0-9._-]*) continue ;; esac
    card_tmp=$(umask 077; mktemp "$dir/.card.XXXXXX") || continue
    if printf '%s\n' "$payload" | jq --arg k "$card_key" \
      '{schema:"fm-decision-card.v1", card:(.captains_call[] | select(.key == $k))}' \
      > "$card_tmp"; then
      chmod 0600 "$card_tmp" && mv -f -- "$card_tmp" "$dir/$card_key.json"
    else
      rm -f -- "$card_tmp"
    fi
  done <<EOF
$(printf '%s\n' "$payload" | jq -r '.captains_call[]?.key // empty')
EOF
}
...
command_build() {
  ...
  # Durable per-card store for out-of-band decision readers (the Captain's Deck
  # Herdr plugin reads state/decision-cards/<task>.json when the live board is
  # absent): every captains_call card that survived this build is written as an
  # fm-decision-card.v1 record keyed by the captain-held task id. Cards whose
  # calls closed stay on disk as the latest written version; the reader falls
  # back to the snapshot row when no card exists.
  write_decision_cards "$json"
