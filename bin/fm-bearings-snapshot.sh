...
MODEL=$(printf '%s' "$SNAP" | jq \
  ...
  | {
      ...
      secondmate_reconcile: [ (.secondmate_current.records // [])[]
        ...
        | {id, spawn_gen:(.spawn_gen // null), host:(.host // null), remote_root:(.remote_root // null), kind:(.reconcile_inventory.kind // null), ids:((.reconcile_inventory.ids // []) | map(select(type == "string")) | sort)} ],
