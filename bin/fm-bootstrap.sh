...
secondmate_reconcile_sweep() {
  ...
  local snapshot out
  snapshot=$("$SCRIPT_DIR/fm-fleet-snapshot.sh" --json 2>/dev/null) || return 0
...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    def verified($h): ["claude","codex","opencode","pi","pi-signed","omp","grok","kimi","hermes"] | index($h);
    def effort_ok($h; $e):
      if $e == null then true
      ...
      elif $h == "pi" or $h == "pi-signed" or $h == "omp" or $h == "hermes" then (["low","medium","high","xhigh","max"] | index($e))
    ...
    elif malformed_optional_fields([(.rules // [])[]? | profiles(.use?)[]?]) then "use profile model, effort, and prewalk_into must be non-empty strings when present"
    ...
    elif has("default") and malformed_optional_fields([profiles(.default)[]?]) then "default profile model, effort, and prewalk_into must be non-empty strings when present"
    else
      (configured_profiles
        | map(.harness)
        | map(select(. != null))
