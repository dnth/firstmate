...
crew_dispatch_validate() {
  ...
  if $typed_active; then
    ...
    verified_harnesses='["claude","codex","opencode","pi","pi-signed","omp","grok","kimi","hermes","devin"]'
  fi
  err=$(jq -r --argjson typed "$typed_active" --argjson verified_harnesses "$verified_harnesses" --arg provider_re "$FM_QUOTA_PROVIDER_ID_RE" '
    ...
    def effort_ok($h; $e):
      ...
      elif $h == "pi" or $h == "pi-signed" or $h == "omp" or $h == "hermes" or $h == "devin" then (["low","medium","high","xhigh","max"] | index($e))
    ...
    def malformed_optional_fields($items):
      ...
      or ($items | any(has("prewalk_into") and (((.prewalk_into | type) != "string") or (.prewalk_into | length) == 0)))
    ...
    def bad_efforts:
      ...
      | map("\(.h):\(.e)")
      | unique;
    def bad_prewalk_harnesses:
      configured_profiles
      | map(select(.prewalk_into != null and .harness != "omp"))
      | map(.harness)
      | unique;
    if type != "object" then "top-level value must be an object"
      ...
      if $typed then "use profile model, effort, and prewalk_into must be non-empty strings when present, and provider must match ^[a-z0-9]+(-[a-z0-9]+)*\\z when present"
      else "use profile model, effort, and prewalk_into must be non-empty strings when present"
      end
      ...
      if $typed then "default profile model, effort, and prewalk_into must be non-empty strings when present, and provider must match ^[a-z0-9]+(-[a-z0-9]+)*\\z when present"
      else "default profile model, effort, and prewalk_into must be non-empty strings when present"
      end
      ...
      | if ($bad_harnesses | length) > 0 then "unverified harness: " + ($bad_harnesses | join(", "))
        elif (bad_prewalk_harnesses | length) > 0 then "prewalk_into requires harness=omp, not: " + (bad_prewalk_harnesses | join(", "))
  ...
  if [ "${FM_BOOTSTRAP_VERBOSE_FACTS:-0}" = 1 ]; then
    ...
    def profile($p):
      ...
      + (if ($p.prewalk_into? != null) then "/prewalk=" + ($p.prewalk_into | tostring) else "" end);
