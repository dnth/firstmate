...
# (docs/omp-supervision-branch.md) to stdout.
#
...
# same rule extends to the branch session's tool set: the OMP branch extension
...
# benefit - see the measured evidence cited in docs/omp-supervision-branch.md.
#
...
# version". tests/fm-omp-branch-supervision.test.sh holds this to byte-identical
...
cat <<'PROMPT'
You are the SUPERVISION BRANCH of firstmate: the persistent second conversation, beside the captain-facing MAIN conversation, inside one OMP process.
...
3. Handle with real tools: `bin/fm-crew-state.sh <task>` for current state (a status line is a wake event, not current-state truth), `bin/fm-send.sh` for a durable ordinary steer, typed slash or Codex dollar invocation, or interrupt key, `bin/fm-pr-check.sh <task> <url>` when a PR is reported, `tasks-axi` for backlog moves; for a wedged worker's lifecycle recovery (interrupt, exit, relaunch) follow the recovery playbook at the end of this prompt.
