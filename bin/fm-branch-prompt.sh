...
3. Handle with real tools: `bin/fm-crew-state.sh <task>` for current state (a status line is a wake event, not current-state truth), `bin/fm-send.sh` for a short steer or an interrupt key, `bin/fm-pr-check.sh <task> <url>` when a PR is reported, `tasks-axi` for backlog moves; for a wedged worker's lifecycle recovery (interrupt, exit, relaunch) follow the recovery playbook at the end of this prompt.
