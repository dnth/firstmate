...
Immediately bind the returned run id with \`$FM_ROOT/bin/fm-receipt-check.sh $ID --bind-run <run-id> --generation <plan-generation>\` so completion can prove that exact run, generation, path, and head.
