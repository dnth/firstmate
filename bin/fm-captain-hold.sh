...
#   fm-captain-hold.sh answers [<legacy-origin> | --any-origin] --source <provenance> [--no-steer]   (keyed answers on stdin)
...
# skipped. The command exits nonzero when any key was
# skipped or a required steer failed. `--source` is provenance text recorded in the
...
# After a key closes - including an idempotent replay - `answers` also steers
# the owning task through bin/fm-send.sh so the captain's words reach the
# worker as a durable inbox record plus the ordinary doorbell. The owner is
# the recorded `Origin:` task when the call was minted for another task's
# review, else the legacy `<origin>-decision-<key>` prefix, else the held task
# itself; a candidate without state/<id>.meta is not a live task and is passed
# over, and a close with no live owner reports `steer-skipped:` rather than
# pretending delivery. A steer that fails after the durable close reports
# `steer-failed:` and fails the command so the caller retries the same keyed
# line. `--no-steer` is the explicit internal exception for a channel that
# already delivered the answer itself (bin/fm-send.sh --resolve-key); it is
# never inferred from provenance.
...
# The status decisions still open for completion purposes on <origin-id>.
# status_open_decisions is the fold's truth; this wrapper keeps the fork's
# single-owner terminal rule upstream applies inside its own fold instead: a
# ship/scout task whose LAST status line is a terminal done/failed declaration
# has no live open set, while a secondmate's terminal line describes other work
# and cannot clear one. fm-captain-hold's `complete` and `verify` read the open
# set through here so a worker's terminal declaration does not wedge the
# completion gate behind a stale keyed decision.
...
# Deliver one closed keyed answer to the task that owns the call, through
# bin/fm-send.sh's ordinary durable-inbox-plus-doorbell path. The owner is the
# recorded `Origin:` task when the call was minted for another task's review,
# else the legacy `<origin>-decision-<key>` prefix, else the held task itself;
# the first candidate with state/<id>.meta wins because fm-send resolves a
# task selector only through recorded metadata. A close with no live owner is
# reported `steer-skipped:` and counts as delivered-by-record alone; a steer
# that fails after the durable close returns nonzero so the caller retries
# the same keyed line rather than reporting a half-delivered answer complete.
# fm-send owns bounded delivery and its non-resend exit semantics, so it runs
# unwrapped with its native status; its durable-but-unproven verdicts (4-7)
# report `steer-deferred:` because the record exists and the watcher's re-ring
# ladder, not this intake, owns redelivery.
# A successful or deferred steer records a private marker under
# state/captain-hold-steers/ keyed by the answer's decision digest, so an
# idempotent replay of the same keyed line does not duplicate the inbox
# record, while a replay after a skipped or failed steer still delivers.
steer_answered_task() {  # <resolved-id> <origin-or-empty> <shown-body> <answer> <label> <decision-digest>
  local id=$1 origin=$2 body=$3 answer=$4 label=$5 digest=$6
  local decoded origin_field prefix candidate target='' msg rc marker mtmp token
  local STEER_ERR
  STEER_ERR=$(umask 077; mktemp "${TMPDIR:-/tmp}/fm-captain-hold-steer.XXXXXX") \
    || { printf 'steer-failed: %s (cannot stage steer diagnostics)\n' "$id" >&2; return 1; }
  decoded=$(decode_shown_value "$body" 2>/dev/null || printf '%s' "$body")
  origin_field=$(printf '%s\n' "$decoded" \
    | sed -n 's/^Origin: \([A-Za-z0-9._-][A-Za-z0-9._-]*\)$/\1/p' | head -1)
  prefix=''
  case "$id" in *-decision-*) prefix=${id%-decision-*} ;; esac
  for candidate in "$origin_field" "$prefix" "$id"; do
    [ -n "$candidate" ] || continue
    [ "$candidate" != "$target" ] || continue
    if [ -f "$STATE/$candidate.meta" ] && [ ! -L "$STATE/$candidate.meta" ]; then
      target=$candidate
      break
    fi
  done
  if [ -z "$target" ]; then
    printf 'steer-skipped: %s (no live owning task in this home)\n' "$id" >&2
    rm -f -- "$STEER_ERR"
    return 0
  fi
  # The delivery token rides inside the inbox message itself, so the durable
  # record is its own dedupe proof: a replay greps the target's inbox -
  # handled records included - for the token before sending again, and the
  # marker under state/captain-hold-steers/ is only the fast path. A marker
  # write failure therefore cannot make a replay duplicate the delivery.
  token="steer-$digest"
  marker="$STATE/captain-hold-steers/$id.steer"
  if { [ -f "$marker" ] && [ ! -L "$marker" ] \
      && [ "$(sed -n 's/^digest=//p' "$marker" | head -1)" = "$digest" ] \
      && [ "$(sed -n 's/^target=//p' "$marker" | head -1)" = "$target" ]; } \
    || { [ -d "$STATE/$target.inbox" ] \
      && grep -rqF "$token" "$STATE/$target.inbox" 2>/dev/null; }; then
    printf 'steered: %s -> %s (already delivered)\n' "$id" "$target" >&2
    rm -f -- "$STEER_ERR"
    return 0
  fi
  msg="Captain answered $id"
  [ -z "$label" ] || msg="$msg ($label)"
  msg="$msg: $answer [$token]"
  local -a steer_env=("FM_HOME=$FM_HOME" "FM_STATE_OVERRIDE=$STATE" "FM_DATA_OVERRIDE=$DATA")
  [ -z "${FM_CONFIG_OVERRIDE:-}" ] || steer_env+=("FM_CONFIG_OVERRIDE=$FM_CONFIG_OVERRIDE")
  rc=0
  env "${steer_env[@]}" "$SCRIPT_DIR/fm-send.sh" "$target" "$msg" \
    </dev/null >/dev/null 2>"$STEER_ERR" || rc=$?
  case "$rc" in
    0)
      printf 'steered: %s -> %s\n' "$id" "$target" >&2
      ;;
    4|5|6|7)
      # fm-send's durable-but-unproven verdicts: the inbox record exists and
      # its contract forbids resending, so the steer is deferred to the
      # watcher's re-ring ladder rather than retried through this intake.
      # Its stderr verdict stays visible for whoever reads this channel's
      # output.
      [ ! -s "$STEER_ERR" ] || cat "$STEER_ERR" >&2
      printf 'steer-deferred: %s -> %s (durable record; fm-send verdict rc=%s)\n' \
        "$id" "$target" "$rc" >&2
      ;;
    *)
      [ ! -s "$STEER_ERR" ] || cat "$STEER_ERR" >&2
      printf 'steer-failed: %s (fm-send to %s exited %s; the answer is durable, retry the keyed line)\n' \
        "$id" "$target" "$rc" >&2
      rm -f -- "$STEER_ERR"
      return 1
      ;;
  ...
  rm -f -- "$STEER_ERR"
  # The delivery is durable; record the fast-path marker. A marker write
  # failure is reported but stays a successful steer: the inbox token grep
  # still dedupes any replay, so no path can double-deliver.
  if (umask 077; mkdir -p "${marker%/*}") && [ -d "${marker%/*}" ] && [ ! -L "${marker%/*}" ]; then
    mtmp=$(umask 077; mktemp "${marker%/*}/.steer.XXXXXX") || mtmp=''
    if [ -n "$mtmp" ]; then
      if {
        printf 'schema=fm-captain-hold-steer.v1\n'
        printf 'task=%s\n' "$id"
        printf 'digest=%s\n' "$digest"
        printf 'target=%s\n' "$target"
        printf 'steered=%s\n' "${FM_CAPTAIN_HOLD_NOW:-$(date -u +%Y-%m-%dT%H:%M:%SZ)}"
      } > "$mtmp" && chmod 0600 "$mtmp" && mv -f -- "$mtmp" "$marker"; then
        :
      else
        rm -f -- "$mtmp"
        mtmp=''
    ...
    [ -n "$mtmp" ] || printf 'steered: %s -> %s (fast-path marker not recorded; inbox token still dedupes replays)\n' \
      "$id" "$target" >&2
  else
    printf 'steered: %s -> %s (fast-path marker not recorded; inbox token still dedupes replays)\n' \
      "$id" "$target" >&2
  fi
}
...
command_answers() {
  ...
  local recorded_digest recorded_mode occurrence tmp err closed=0 skipped=0 steer_failed=0 no_steer=0 reason release_flag tab=$'\t'
  local resolve_rc
  while [ "$#" -gt 0 ]; do
    case "$1" in
  ...
  while IFS= read -r row; do
    ...
    if body_has_resolution_record "$body" \
      ...
      if { [ -z "$release_flag" ] && [ "$state" = "done" ] \
        ...
        closed=$((closed + 1))
        if [ "$no_steer" != 1 ] \
          && ! steer_answered_task "$id" "$origin" "$body" "$answer" "$label" "$digest"; then
          steer_failed=$((steer_failed + 1))
        fi
        continue
      fi
    fi
  ...
  rm -f -- "$tmp" "$err"
  printf 'answers: closed=%s skipped=%s steer-failed=%s\n' "$closed" "$skipped" "$steer_failed"
  [ "$skipped" -eq 0 ] && [ "$steer_failed" -eq 0 ]
}
...
command_complete() {
  local origin=${1:-} meta previous='' supplied='' keys='' entry key status_file open has_meta=0 transfers=() resolved
...
EOF
  ...
  # An open status decision must be covered by the attested inventory before it
  # is transferred: a captain-held close with no matching held task would read
  # as transferred while nothing durable holds the call (this fork's check,
  # kept from the retired fm-decision-hold.sh). A status key is covered by the
  # task id itself or by its legacy derived <origin>-decision-<key> identity.
  if [ -n "$open" ] && [ -n "$keys" ]; then
    while IFS=$'\t' read -r key _verb _summary; do
      [ -n "$key" ] || continue
      list_has_key "$keys" "$key" || list_has_key "$keys" "$origin-decision-$key" \
        || fail "open captain decision $origin/$key has no captain-held inventory entry; hold a captain task for it, or answer it, before completing"
    done <<EOF
$open
EOF
  fi
  ...
  if [ "$has_meta" = 1 ]; then
    ...
    # Call item. The transfer iterates the RAW fold: a terminal-superseded key
    # is still open to every status reader, so its captain-held close must land
    # even though it could never block the refusal above. This fork's status
    # lines carry no emission stamps and its watcher has no self-announced-append
    # ledger, so the transfer lines go in with a plain append (the retired
    # fm-decision-hold.sh's behavior); an append failure still fails loudly.
    if [ -n "$keys" ]; then
      while IFS=$'\t' read -r key _verb _summary; do
        [ -n "$key" ] || continue
        list_has_key "$keys" "$key" || list_has_key "$keys" "$origin-decision-$key" || continue
        transfers+=("captain-held [key=$key]: tracked by $keys")
      done <<EOF
$raw_open
EOF
      if [ "${#transfers[@]}" -gt 0 ]; then
        printf '%s\n' "${transfers[@]}" >> "$status_file" \
          || fail "cannot append the captain-held transfer for $origin"
      fi
    fi
  fi
