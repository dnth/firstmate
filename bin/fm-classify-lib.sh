...
# Keep the worktree probe's hard bound local to this library using the same
# timeout/gtimeout/Perl selection as the bounded snapshot and vendor-auth readers.
_fm_classify_run_timed() {  # <seconds> <command...>
  local seconds=$1
  shift
...
# The 45-minute (2700s) finite anti-rot recheck is short enough that a forgotten
# hold cannot rot invisibly and far longer than FM_STALE_ESCALATE_SECS (default
# 240s), so wedge detection is unchanged. Both consumers read
...
# still in flight: the generated status contract (bin/fm-brief.sh) requires a
# terminal or paused status line within this many seconds of turn-end, and
# bin/fm-watch.sh escalates a still-silent idle pane past it as
# `idle-with-open-work`. Ten minutes sits comfortably above a normal
# turn-boundary report while keeping a silent idle pane visible. The watcher
# reads FM_IDLE_OPEN_WORK_SECS with this default and the brief generator quotes
# the same number, so the bound has one owner.
# shellcheck disable=SC2034 # Read by fm-watch.sh and fm-brief.sh, not this lib.
FM_IDLE_OPEN_WORK_SECS_DEFAULT=600
...
ship_done_evidence_gate() {  # <task-id> <kind>
  ...
  if command -v jq >/dev/null 2>&1 && printf '%s' "$out" | jq -e . >/dev/null 2>&1; then
    ...
    invalid=$(printf '%s' "$out" | jq -r '.invalid | join("; ")')
    if [ -n "$invalid" ]; then
      printf 'invalid evidence: %s' "$invalid"
    elif [ -n "$missing" ]; then
      printf 'missing evidence: %s' "$missing"
    else
...
# bin/fm-pr-lib.sh owns the canonical PR/MR URL grammar; source it on first use
# so consumers that never check a ship done: claim never pay for it.
...
# 0 when the given done: status line carries the delivery artifact its
# kind/mode contract requires, so a bare `done:` can never be accepted as a
# delivery claim. The worker-facing contract is generated by bin/fm-brief.sh;
# this predicate is the single machine check behind both
# bin/fm-receipt-check.sh --complete and bin/fm-teardown.sh's refusal:
#   ship, PR modes:   a canonical PR/MR URL token (fm-pr-lib.sh's grammar)
#   ship, local-only: the "ready in branch" marker
#   scout:            the report path data/<id>/report.md - an absolute path to
#                     the same report ends with that tail
# Other kinds carry no done:-artifact contract and return 0.
status_done_line_has_artifact() {  # <done-line> <kind> <mode> <id>
  ...
  case "$kind" in
    ...
    ship)
      ...
      _fm_classify_require_pr_lib
      urls=$(printf '%s\n' "$line" | grep -oE "https?://[^[:space:]()<>'\"\`]+" || true)
      while IFS= read -r token; do
        # Strip trailing prose punctuation; a canonical PR/MR URL ends in digits.
        while [ -n "$token" ]; do
          case "$token" in
            *[A-Za-z0-9/]) break ;;
            *) token=${token%?} ;;
        ...
        [ -n "$token" ] || continue
        fm_pr_url_parse "$token" 2>/dev/null && return 0
      done <<< "$urls"
      return 1
      ;;
    *) return 0 ;;
...
# Bump FM_OPEN_DECISIONS_FOLD_VERSION whenever _fm_decision_fold_line semantics
# change so older persisted interpretations are rebuilt from byte 0.
#
...
#
# The other real failure mode is OUR OWN read failing (a stat/wc/span-read I/O
...
crew_worktree_written_since() {  # <id> <state> <anchor-file>
  ...
  if [ "${#prune[@]}" -gt 0 ]; then
    hit=$(_fm_classify_run_timed "$bound" find "$wt" -xdev -maxdepth "$FM_WORKTREE_WRITE_MAXDEPTH" \
