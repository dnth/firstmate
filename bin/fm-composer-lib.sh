...
fm_composer_omp_steering_block_count() {  # <composer-top-row>
  local top_row=$1 row=0 line plain trimmed state=seek count='' expected=0 item=''
  while IFS= read -r line; do
    row=$((row + 1))
    [ "$row" -le "$top_row" ] || break
    plain=$(printf '%s' "$line" | fm_composer_strip_ansi)
    trimmed="${plain#"${plain%%[![:space:]]*}"}"
    trimmed="${trimmed%"${trimmed##*[![:space:]]}"}"
    if [ "$row" -eq "$top_row" ]; then
    ...
    case "$state" in
      items)
        item=$(printf '%s\n' "$trimmed" | sed -nE 's/^([1-9][0-9]*)\.[[:space:]]+.+$/\1/p')
        ...
        ;;
      hint)
        if [ "$trimmed" = '└ Alt+Up/Shift+Up to edit' ]; then
          state=status
          continue
        fi
        state=seek
