...
# record before it closes a hold.
#
...
#
# `resolve` and `answer` close active holds. Both paths require a non-empty
# captain decision file of at most 8192 bytes, record the same durable resolution
# block in the hold body, and store the decision digest plus routed identities so
# an exact retry is idempotent while a changed decision or, for `resolve`, routed
# set is rejected. New records include a `Resolution mode:` naming their path;
# older routed records remain valid.
#
...
# follow-up work has been routed behind the hold yet, and closes it. It takes no
# --routed-to task and refuses while any task is still blocked by the hold, so a
# decision whose follow-up work is already routed still goes through `resolve`
# and the routed-vs-unrouted distinction survives. The unrouted close is one
# shared implementation (close_unrouted_hold) parameterized only by the recorded
# resolution mode, so a later unrouted path such as upstream's `decline` cannot
# drift into a weaker close than `answer`.
#
...
# routed-versus-unrouted, or close a hold itself. A future channel needs no change
# here at all.
#
...
command_resolve() {
  ...
  [ -n "$routed" ] || fail "at least one --routed-to task is required; use answer when the captain's answer routes no work yet"
  routed=$(printf '%s\n' "$routed" | tr ' ' '\n' | sed '/^$/d' | LC_ALL=C sort -u | paste -sd' ' -)
  routed_csv=$(printf '%s\n' "$routed" | tr ' ' ',')
  require_tasks_axi
  id=$(hold_id "$origin" "$key")
...
# The one unrouted close path. `answer` is its only caller today; upstream's
# `decline` shares it and differs only in the resolution mode it records and
# the outcome word it prints. Every guard - the captain decision file, the
# active-hold requirement, the retry identity, and the refusal to release
# still-routed work - lives here once, so no caller can drift into a weaker
# close than another.
close_unrouted_hold() {  # <mode> <outcome-word> <origin-id> <decision-key> <flag-args...>
  ...
  [ "$state" != "done" ] \
    || fail "captain hold $id was closed outside fm-decision-hold and carries no captain decision record"
  verify_hold_active "$id"
  hold_body=$(show_field "$hold_show" body)
  case "$hold_body" in
    *"Resolution recorded by fm-decision-hold."*)
