...
# Millisecond clock without fm-timing-lib (absent in this tree): prefer GNU
# date %3N, fall back to whole seconds so latency stays a number.
now_ms() {
  local ms
...
VERIFIED_HARNESSES=$(fm_dispatch_harnesses | jq -Rsc 'split("\n") | map(select(length > 0))')
...
# effort_ok mirrors the crew_dispatch_validate table in bin/fm-bootstrap.sh;
# prewalk_into follows the same omp-only rule bootstrap enforces.
rules_err=$(jq -r --argjson verified_harnesses "$VERIFIED_HARNESSES" --arg provider_re "$FM_QUOTA_PROVIDER_ID_RE" '
  ...
  def effort_ok($h; $e):
    ...
    elif $h == "pi" or $h == "pi-signed" or $h == "omp" or $h == "hermes" or $h == "devin" then (["low","medium","high","xhigh","max"] | index($e)) != null
  ...
  def profile_bad($p):
    ...
    or ($p | has("prewalk_into") and ((.prewalk_into | type) != "string" or (.prewalk_into | length) == 0))
  ...
  elif any((.rules // [])[] | profiles(.use)[]; profile_bad(.)) then "each use profile needs harness; model, effort, prewalk_into, and floor must be well formed, and provider must match ^[a-z0-9]+(-[a-z0-9]+)*\\z when present"
  ...
  elif any((.rules // [])[] | profiles(.use)[]; (.prewalk_into != null and .harness != "omp")) then "prewalk_into requires harness=omp"
  elif any((.rules // [])[] | profiles(.use)[]; (verified(.harness) | not)) then "each use profile must name a verified harness"
  elif any((.rules // [])[] | profiles(.use)[]; (effort_ok(.harness; .effort) | not)) then "each use profile effort must be supported by its harness"
  elif has("default") and (profiles(.default) | length) == 0 then "default must be a profile object or non-empty profile array"
  elif has("default") and any(profiles(.default)[]; profile_bad(.)) then "each default profile needs harness; model, effort, prewalk_into, and floor must be well formed, and provider must match ^[a-z0-9]+(-[a-z0-9]+)*\\z when present"
  ...
  elif has("default") and any(profiles(.default)[]; (.prewalk_into != null and .harness != "omp")) then "prewalk_into requires harness=omp"
  elif has("default") and any(profiles(.default)[]; (verified(.harness) | not)) then "each default profile must name a verified harness"
  elif has("default") and any(profiles(.default)[]; (effort_ok(.harness; .effort) | not)) then "each default profile effort must be supported by its harness"
  else empty end
...
TEXT=$(jq -r '
  ...
  (if .chosen then "  profile: --harness \(.chosen.profile.harness | shell_arg)"
      ...
      + (if .chosen.profile.prewalk_into then " --prewalk-into \(.chosen.profile.prewalk_into | shell_arg)" else "" end) else empty end)' <<<"$RESULT") || emit_error "output rendering failed"
printf '%s\n' "$TEXT"
exit 0
