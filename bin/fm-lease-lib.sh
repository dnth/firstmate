...
#
# WHY. On the OMP supervision branch (docs/omp-supervision-branch.md), two LLM
# actors share one firstmate home inside one OMP process: MAIN (the captain's
...
#     FM_SUPERVISION_ACTOR=branch injected deterministically by the OMP branch
...
#     both actors live inside that one OMP process, so a dead recorded pid
...
# running as the same uid inside the same OMP process can evade any in-process
...
# the actor variables readonly (see the OMP branch extension), so an
...
#     refuses with exit FM_LEASE_REFUSE_EXIT. In a OMP supervision context the
...
#     home without the current OMP session lock cannot have a live lease, so
#     the guard is a no-op there - non-supervision behavior is unchanged by construction.
...
# fm_lease_live <task>: 0 iff a well-formed lease exists in a coding-agent context, its
# recorded pid is alive, and that pid IS the current session-lock holder (see
# the staleness contract above).
fm_lease_live() {
  local lock_pid
  # PI_CODING_AGENT is the ambient coding-agent-context marker: the Pi runtime
  # sets it natively, and the OMP branch extension sets it at load (OMP does not
  # set it on its own). A live lease additionally proves this task is inside the
  # two-actor contract; an explicit actor covers OMP calls before a lease exists.
...
# a live lease held by the OTHER actor exists for <task>. In an OMP supervision
...
# This closes the check/use race with a concurrent claim. Outside a supervision context, stale
...
fm_lease_guard() {
  ...
  case "${FM_SUPERVISION_ACTOR:-}" in
    main|branch) active=1 ;;
  esac
...
fm_lease_forbid_branch() {
  ...
  echo "error: $action refused - the supervision branch never performs this action; report the outcome and leave it to main (role partition: docs/omp-supervision-branch.md)" >&2
  exit "$FM_LEASE_REFUSE_EXIT"
}
