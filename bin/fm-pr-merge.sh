...
# bin/fm-pr-lib.sh also parses GitLab merge request URLs so the watcher can
# follow them, but this path still addresses only GitHub by owner and
# repository and refuses a GitLab URL until merge parity lands.
#
  ...
  for arg in "$@"; do
    case "$arg" in
      --repo|--repo=*)
        ...
        ;;
      --*) ;;
      # A single-dash argument is a short-option cluster, which the CLI expands
...
# Reached only after the forge confirmed the merge landed: every refusal above
# exits non-zero, and a queued merge exits without an outcome while its
# existing poll remains armed.
outcome_rc=0
