...
cmd_send() {
  ...
  if [ "$harness" = omp ]; then
    ...
    FM_SEND_RECONCILE_AUTH=1 FM_SEND_REMOTE_TYPED=1 FM_SEND_PRESERVE_INBOUND_FROM_FIRSTMATE=1 FM_HOME="$TARGET_HOME" FM_ROOT_OVERRIDE="$FM_ROOT" FM_STATE_OVERRIDE="$CONTROL_STATE" FM_DATA_OVERRIDE="$CONTROL_DATA" \
      "$SCRIPT_DIR/fm-send.sh" "$id" "${send_args[@]}" "$message"
  fi
}
