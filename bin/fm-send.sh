...
if [ "${1:-}" = "--key" ]; then
  ...
  if [ "$TARGET_BACKEND" = remote ]; then
    ...
    if [ -n "$FIRE_AND_FORGET_ID" ]; then
      remote_out=$("$SCRIPT_DIR/fm-on.sh" "$TARGET_REMOTE_ID" fm-remote-secondmate-control.sh send "$TARGET_REMOTE_ID" "$MESSAGE" fire-and-forget reconcile < /dev/null 2>&1) || send_rc=$?
