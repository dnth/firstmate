...
#                       convergence, secondmate liveness, pending remote
#                       handoff retry, X-mode artifact writes, ext-bridge
#                       artifact writes, and fleet sync) also run only when
#                       locked.
...
#                       its reads always run; its board-drift check reconciles
#                       only with verified lock ownership and is report-only
#                       after lock refusal.
...
#
# Those eight names are also the digest's logical order, so fleet state stays
# visible ahead of curated memory when a harness truncates an oversized output.
#
...
# and it has really been truncated in practice. What a truncated tail drops
# must therefore be the CHEAPEST thing to lose. Curated memory is stable session
# to session, is already governed by a captain-set budget, and is recoverable
# with one targeted read; live fleet identity - which tasks exist, their
# windows, worktrees, backends, and endpoint liveness - changes every session
# and is exactly what recovery depends on. So fleet state goes first and the
# memory files absorb the truncation. The read-once contract moves ahead of both
# for the same reason: a contract that only arrives after the payload it governs
# is the first thing a truncated digest loses.
#
...
#
# On Pi-family and OMP primaries, the supervision-block step also validates the
# tracked native integration marker against the lock-owning process. It prints
# a harness-specific recovery line when the required integration is missing or
# stale.
#
# Why lock first: the old documented order (bootstrap, THEN lock) let a
# SECOND concurrent session run bootstrap's mutating sweeps before discovering
# another session already held the lock. Two sessions racing those sweeps is
...
# go dark. So on refusal, bootstrap still runs in detect-only mode for its
# read-only diagnostics, while the context and fleet-state digests below remain
# safe to run unconditionally in both modes.
#
#
...
#     reporting surfaces, and at startup it is pure weight.
...
# `tasks-axi ready`), so this script never reimplements task state.
...
# rule applies, recognized there from the title line's own markers.
...
print_status_tail() {
  ...
  # A crewmate writes its own status lines, so their length is unbounded.
  # Cap each one the way the wake digest's OPEN DECISIONS section does.
...
# next turn from re-reading everything the digest just printed.
...
# Before CONTEXT: live fleet identity is what a truncated tail must never take.
section "FLEET STATE"
...
# session, already governed by the startup-memory budget, and recoverable with
# one targeted read, so it is the cheapest thing for a truncated tail to take.
section "CONTEXT"
