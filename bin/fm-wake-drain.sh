#!/usr/bin/env bash
# Present durable watcher wake records, retire rows no actor could ever
# consume, and the folded OPEN DECISIONS and unread informational-status
# sections, fail closed if that presentation cannot complete, optionally
# acknowledge handled records, annotate every unread line for validated signal
# status keys, then assert liveness.
#
# Usage:
#   fm-wake-drain.sh
...
# deterministically by the OMP branch extension's bash tool - never agent
# memory) drains and acks only the row set the extension granted to it.
# .omp/extensions/lib/fm-branch-dispatch.ts is the single owner of that
# eligibility classification; this script never reclassifies a row itself, it
# only consumes the extension's already-computed verdict, published to
# ELIGIBLE_ROWS_FILE under the queue lock immediately before every branch
# prompt. A row whose sequence number is not in that file is left completely
# untouched by a branch-actor drain or ack, so the branch's ack can never
# swallow a main-owned row still waiting for main.
#
# INERT BY CONSTRUCTION: a home that never runs the supervision branch has none
# of these actor files. SCOPED (decided under the queue lock below) then stays
# false and every path takes the byte-behavior-identical pre-branch route - no
# actor state is written, no actor logic runs (AGENTS.md; the inert-by-default
# test).
...
# SCOPED is decided once, under the queue lock, from the actor files present at
# that moment. A branch actor is always scoped (it must name its snapshot); a
# main actor is scoped only while the branch mechanism is, or recently was, in
# play. Callers set it after acquiring FM_WAKE_QUEUE_LOCK.
...
print_status_presentation() {  # [<deduped-raw-rows>]
  ...
  if [ "$rc" -eq 0 ] && [ -n "$rows" ] && [ -n "$snapshot" ]; then
...
if [ "$SCOPED" = true ]; then
  ...
  # A stale branch grant may have just been reclaimed; keep the branch's own
  # snapshot requirement in force under the lock.
  [ "$ACTOR" != branch ] || require_branch_eligible_rows || exit 1
fi
# Main also owns retiring rows no actor could ever present or acknowledge; the
# branch may only consume the sequences its live grant names.
...
if [ -n "$ACK_THROUGH" ]; then
  ...
  elif [ "$ACTOR" = branch ]; then
    require_branch_eligible_rows || exit 1
    # Delete a row only when its sequence is <= cutoff AND named in the
...
if [ ! -s "$FM_WAKE_QUEUE" ]; then
  ...
  (print_status_presentation) || {
    echo "wake drain: status could not be presented safely" >&2
    exit 1
  }
...
if [ "$SCOPED" = true ] && [ "$ACTOR" = main ]; then
  claim_main_rows_locked || exit 1
  if [ ! -s "$MAIN_ROWS_FILE" ]; then
    # Every currently queued row is granted to the branch; main has nothing to
    # present or acknowledge this turn. Say so rather than exiting silently: a
    ...
    # Still surface open decisions and assert liveness so a mixed queue never
    # hides main's own supervision health.
    print_branch_held_notice
    fm_lock_release "$FM_WAKE_QUEUE_LOCK"
    DRAIN_LOCK_HELD=false
    (print_status_presentation) || {
...
if [ "$SCOPED" = true ]; then
  # Present and bound the acknowledgement to only this actor's own rows. main's
  # owned set is claim_main_rows_locked's output; branch's is its eligible
  # snapshot. A row outside the actor's set is never presented and never
  # acknowledged here, so a mixed queue splits cleanly between the two.
  if [ "$ACTOR" = branch ]; then
...
(print_status_presentation "$RAW_ROWS") || {
  echo "wake drain: status could not be presented safely" >&2
  exit 1
}
assert_watcher_liveness
