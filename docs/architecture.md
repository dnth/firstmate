...
The drain coordinates that fold and its annotations through a locked fleet-wide snapshot whose `.status-presentation-cursor` manifest records each status file's identity, last-presented byte offset, and the separate backstop receipt offset.
...
On the main actor, the same drain adds a bounded STATUS OUTCOME BACKSTOP for the newest captain-facing event after its receipt offset when no newer branch outcome with matching status provenance identity covers that event; acknowledging the section advances only that receipt, so routine appends cannot hide or replay the event.
