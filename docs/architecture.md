...
For an OMP worker the loaded extension delivers the doorbell through `sendMessage` and claims it delivered only when a `turn_start`/`agent_start` proves the triggered turn ran; the generated worker forwards its own correlated `turn_start`/`turn_end` events to the doorbell, which keeps the doorbell off a second `omp.on` subscription while preserving turn proof and closing stale turn state.
When the runtime downgrades `triggerTurn` to append-only, the extension re-drives the instruction through `sendUserMessage` after a bounded grace rather than leaving an idle session stranded (`.omp/extensions/lib/fm-task-inbox-doorbell.ts`).
