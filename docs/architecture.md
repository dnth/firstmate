...
Pane existence, busy checks, composer checks, capture, and verified submit route through `bin/fm-backend.sh`: tmux keeps the same submit core used by the tmux send backend, while herdr, for a Claude pane or an idle OMP away-mode supervisor, types only into a verified-empty composer and withholds Enter until that composer provably holds the typed payload, and then prefers native agent-state confirmation with its ANSI-aware structural composer classifier as the fallback signal.
