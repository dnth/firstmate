...
A remote host may optionally be an ephemeral rented pod instead of a permanent machine: `bin/fm-runpod.sh` owns that compute lifecycle beneath the unchanged remote mechanism, `bin/fm-runpod-lib.sh` is the cheap predicate that lets supervision, convergence, and delivery tell a deliberately suspended route from a broken one, and [`runpod-secondmates.md`](runpod-secondmates.md) owns the whole contract.
