...
The tmux submit core (shared `fm_tmux_submit_enter_core`) treats a busy pane plus retries-exhausted plus composer-still-pending as a queued Enter for non-OMP harnesses that expose that verified behavior, reported as `empty` so the daemon and `fm-send` do not re-send; an idle pane keeps the `pending` verdict as a genuine swallow.
An already-busy OMP pane is excluded from that inference on both tmux and Herdr: only a task-bound valid composer whose `Steering · N` count increases from a snapshot taken after typing and immediately before Enter can supplement native delivery proof, while tmux retains its separate initially-idle to busy transition proof.
