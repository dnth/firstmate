...
When the runtime downgrades `triggerTurn` to append-only, the extension re-drives the instruction through `sendUserMessage` only after its bounded grace expires without any turn opening (`.omp/extensions/lib/fm-task-inbox-doorbell.ts`).
