...
On the main actor, the same drain adds a bounded STATUS OUTCOME BACKSTOP for the newest captain-facing event after its receipt offset when no newer branch outcome covers that event; acknowledging the section advances only that receipt, so routine appends cannot hide or replay the event.
