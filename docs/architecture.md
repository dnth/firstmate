...
When the runtime downgrades `triggerTurn` to append-only, the extension re-drives the instruction through `sendUserMessage` only after its bounded grace expires without any turn opening; the re-drive is itself only a request, so a nonthrowing return is never a receipt - the entry re-parks for one more bounded proof window and re-queues the durable request when no turn opens (`.omp/extensions/lib/fm-task-inbox-doorbell.ts`).
