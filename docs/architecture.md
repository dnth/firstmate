...
OMP is excluded from that inference on both tmux and Herdr: only a task-bound valid composer whose `Steering · N` count increases from a pre-Enter snapshot can supplement native delivery proof.
