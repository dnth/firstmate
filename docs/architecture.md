...
The tmux submit core (shared `fm_tmux_submit_enter_core`) treats a busy pane plus retries-exhausted plus composer-still-pending as a queued Enter on its non-OMP path, currently verified for OpenCode 1.18.4, reported as `empty` so the daemon and `fm-send` do not re-send; an idle pane keeps the `pending` verdict as a genuine swallow.
