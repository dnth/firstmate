...
Before either stale wake publishes, `bin/fm-stall-recovery.sh` runs a custody-checked bounded auto-recovery: it re-proves the record is still the oldest unhandled instruction and classifies the endpoint. A live endpoint always escalates - recovery never interrupts, exits, or relaunches a session that exists - while a positively missing endpoint may relaunch after proving a clean non-run crew-state and the durable `fm-<id>` Treehouse lease, re-proving the whole chain inside fm-control's lifecycle lock immediately before `fm-control.sh relaunch`. The missing-endpoint path needs no busy-state proof: there is no live worker to interrupt.
