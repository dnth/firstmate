...
The remote-secondmate publisher is serialized against retirement by the secondmate registry lock instead, which the remote teardown path holds across removing the route and retiring the metadata, and the publisher's registry re-read under that lock refuses once the route is gone.
