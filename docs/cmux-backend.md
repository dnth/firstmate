...
- `fm_backend_cmux_endpoint_absent` gives `fm-spawn --relaunch` a structural absence proof - the recorded workspace or its surface verifiably gone, while a live task-titled workspace still counts as present even when the recorded UUID is stale - which licenses recreating the workspace inside the recorded worktree after the task's lease ownership is proven; a live endpoint still refuses because its only cwd read would inject a probe, and every unverifiable shape refuses.
