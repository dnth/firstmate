...
A new pick does not rebuild the branch already running: mid-flight model or effort change is out of scope, so the pin takes effect at the next Firstmate start, when a fresh process rebuilds the branch (see [docs/omp-supervision-branch.md](omp-supervision-branch.md)).
