...
OMP discovers native project extensions from the launch cwd's `.omp/extensions` directory and from extension roots declared by `.omp/settings.json`, separately from its profile-scoped home extensions.
