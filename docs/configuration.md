...
After normal inherited configuration reaches a RunPod remote home, the provider layer writes that home's `config/crew-harness` as `codex`, writes `config/crew-harness-fallback` as `claude`, and removes an inherited dispatch profile that would bypass that primary-to-fallback contract.
Those files govern the RunPod second mate's crews only, while `config/secondmate-harness` in the primary home continues to select the second-mate agent itself.
RunPod boot also maintains the durable account's `.profile`, `.bashrc`, `.claude.json`, and `.claude/settings.json` for its login-shell PATH and unattended root-sandbox Claude contract.
