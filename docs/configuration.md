...
## RunPod compute lifecycle (config/runpod.env, config/runpod/, data/runpod/)

RunPod is an optional compute lifecycle provider beneath a whole-home remote second mate, never a session backend.
[`runpod-secondmates.md`](runpod-secondmates.md) owns the cost model, storage model, operator sequence, and safety behavior, and `bin/fm-runpod.sh`'s header owns every subcommand, flag, and record field.

`config/runpod.env` is a local, gitignored, mode-600 file in ordinary KEY=value form holding `RUNPOD_API_KEY`; it is parsed rather than sourced, is not inherited by secondmate homes, and every command that needs the API refuses with that path and key name before making any request.
`config/runpod/ssh.d/<alias>.conf` and `config/runpod/known_hosts` are generated by `bin/fm-runpod.sh`; wire the fragments in with one `Include` line at the top of the operator's own `~/.ssh/config`.
`config/runpod/omp-auth-broker.token` is the local, gitignored, mode-600 copy of the existing OMP broker bearer that `bin/fm-runpod-omp-auth.sh` refreshes without rotation before an authenticated pod wake.
`state/runpod-omp-auth/` holds private workstation supervisor identity records and logs for the loopback broker, credential-read-only facade, and per-pod reverse tunnels.
`data/runpod/<id>.meta` is the authoritative local record binding a second mate to its network volume, datacenter, current pod, current endpoint, compute type, and lifecycle state.
`state/.runpod-lifecycle-<id>.lock` serializes provider lifecycle mutations for one second mate; `bin/fm-runpod.sh`'s header owns the exact command set.
...
Lifecycle transitions and other wrapper-backed firstmate backlog commands therefore run through [`bin/fm-tasks-axi.sh`](../bin/fm-tasks-axi.sh), which addresses this home's backlog and archive from any working directory exactly as lifecycle transitions do, and bootstrap reports a code-root `data/backlog.md` or `data/done-archive.md` that is not this home's own file as a `BACKLOG_RECONCILE: code-root ...` line even in a read-only session.
...
Ordinary spawns with an authoritative local Treehouse pool lease and screen a slot before creating an endpoint, refusing slots carrying `.fm-secondmate-home` or `.fm-secondmate-parent`; other acquisition paths keep their endpoint-guarded check. Sandbox spawns perform the same guarded refusal as the first command inside the endpoint and remove that endpoint before any agent launch when the marker check fails.
...
A slot still carrying `.fm-secondmate-home` or `.fm-secondmate-parent` residue from a retired secondmate is reported as dirty with the marker named, because those gitignored files are invisible to the porcelain check but would make an ordinary task placed there read as a secondmate home.
...
OMP remains a separate `harness=omp` identity, must resolve to either a Bun script or a Bun-compiled standalone executable with every required launch and resume capability, and never falls back to Pi or another executable.
Its primary loaded marker retains the adapter hash, owning PID, canonical runtime identity, and canonical OMP entrypoint identity; standalone OMP records the same executable in both identity slots, and legacy two-line OMP markers cannot authorize recovery.
...
Every non-secondmate `fm-spawn` launch (ship, scout, prewalk, relaunch) stamps `FM_TASK_ID=<id>` on the launch command itself, so the worker's harness and every subprocess it starts inherit the marker; `bin/fm-primary-scope-lib.sh` treats it as the worker-identity boundary, and `bin/fm-session-start.sh`, `bin/fm-wake-drain.sh`, `bin/fm-watch-arm.sh`, and `bin/fm-watch.sh` refuse that environment outright so a task worker can never lock, drain, or supervise a firstmate home.
`fm-spawn.sh` likewise refuses to launch a non-secondmate task into a worktree still carrying `.fm-secondmate-home` or `.fm-secondmate-parent`, and `bin/fm-teardown.sh` strips both markers from every slot it returns to the pool.
...
A crew-only harness is ineligible for that implicit fallback: when `config/crew-harness` holds `hermes` or `devin` and `config/secondmate-harness` is absent or `default`, the chain skips it and continues to the primary's own harness, so a Hermes or Devin crew fleet still launches secondmates without needing a new setting.
An explicit `config/secondmate-harness: hermes` or `devin` stays authoritative and is refused at the spawn kind boundary with a precise diagnostic rather than being silently rewritten.
...
For Devin crews, `fm-spawn.sh` writes one native project-local `.devin/config.local.json` Stop registration, installs its guarded script and registry under `${XDG_CONFIG_HOME:-$HOME/.config}/devin/`, drops a per-task `.fm-devin-turnend` pointer in the worktree, and records the matching private registry token and directory for teardown.
It also writes a private mode-0600 per-worker config `state/<id>.devin-config.json` through `bin/fm-devin-config.sh` and launches with `--config` pointed at it: the file preserves the operator's Devin settings but forces `read_config_from.claude=false` so the worker cannot inherit the host's Claude hooks and `attribution=false` so its commits carry no agent co-author trailer, a failed write refuses the launch, and teardown removes the file.
...
Firstmate's tracked `.omp/config.yml` keeps OMP's todo tool enabled but disables its end-of-turn incomplete-todo reminders because the session todo is a projection of long-lived fleet work rather than work that must finish within the current captain turn.
This prevents OMP from scheduling a second model turn after a captain-facing answer solely because projected fleet work remains incomplete.
  ...
  "rules": [
    {
      ...
      "use": [
        { "harness": "<adapter>", "model": "<optional model>", "effort": "<low|medium|high|xhigh|max, optional>", "prewalk_into": "<OMP target model spec, optional>", "provider": "<optional quota-axi provider>", "floor": { "scope": "<quota-axi scope>", "min_percent": 50 } }
      ],
...
It is off unless `TYPESAFE_API_KEY` is non-empty in the calling environment or the home's gitignored `.env` holds a `TYPESAFE_API_KEY=` line; the environment wins, matching the Relay contract, and the Relay accessor in `bin/fm-env-lib.sh` reads the line.
...
Typed resolution reads a schema-5 `quota-axi --json` snapshot through `fm_quota_json_valid`; a snapshot from a build older than the schema-5 producer is an ordinary `error` outcome that returns the intake to firstmate, so the `FM_QUOTA_AXI_MIN` floor is unchanged.
...
printf '%s\n' 'hermes:telegram:-1001234567890:17585' \
  >> config/inbox-result-targets
...
bin/fm-inbox-result.sh retry --note-id <note-id>

# A permanent configuration refusal additionally requires --force after repair.
...
The (condition, action) spec is stored privately under `state/when/` and hash-bound by a trust record the same way `bin/fm-check-register.sh` binds a custom check, while the spec separately binds both resolved executable byte hashes; a mutated or unregistered spec or a changed executable is refused before that executable runs.
...
Some sources carry the captain's answer to a durable captain-held task, and what such an answer means is owned once by `bin/fm-captain-hold.sh`'s keyed-answer intake rather than by any channel.
A source bound with `bin/fm-captain-hold.sh bind <source-id> [<origin-id>]` therefore has each captured result passed to `bin/fm-procevent-<adapter>.sh answers <result-file>`, and whatever that prints is piped straight into that intake.
Reconcile selections use the parallel `reconciles` adapter command and the binding-verified `reconcile-requests` intake, never the keyed-answer value.
...
FM_BACKEND_HERDR_IDLE_RE=      # herdr-only: empty-composer placeholder regex after shared ghost extraction plus border and prompt stripping; unset inherits the fleet-wide FM_COMPOSER_IDLE_RE default below
...
FM_WATCH_RESURFACE_MAX_ANNOUNCEMENTS=3   # unacknowledged downtime announcements before an armed watcher resumes pane supervision instead of exiting on the resurface wake; invalid values use the default (docs/watcher-continuity.md "Recovery episode acknowledgement")
FM_WATCH_RESURFACE_MAX_SECS=900   # seconds since the first unacknowledged downtime announcement after which the same bound trips; invalid values use the defaults
...
FM_COMPOSER_IDLE_RE=    # optional override for the fleet-wide empty-composer placeholder regex, applied after ghost and border stripping. Unset installs the verified default alternation in bin/fm-composer-lib.sh, which treats OMC's lone `✻` empty prompt glyph and Hermes' busy `Ctrl+C to interrupt…` placeholder as empty composer content so a retried Enter cannot read them as pending typed input; it applies to both the tmux and herdr composer readers, and FM_BACKEND_HERDR_IDLE_RE inherits it
...
FM_TASK_INBOX_RING_MAX=3     # watcher doorbell attempts before custody-checked stall recovery runs; an ordinary stale wake follows only when recovery escalates or the helper fails, while deferred and recovered outcomes suppress it
