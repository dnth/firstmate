...
Ordinary non-sandbox spawns lease and screen a Treehouse slot before creating an endpoint, refusing slots carrying `.fm-secondmate-home` or `.fm-secondmate-parent`; sandbox spawns perform the same guarded refusal as the first command inside the endpoint and remove that endpoint before any agent launch when the marker check fails.
