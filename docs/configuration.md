...
It rejects generic command carriers, common destructive or remote-control commands, and direct or byte-identical aliases of this fork's RunPod, X-mode, and public-followup entrypoints before registration, so the primitive cannot turn those guarded surfaces into unattended condition or action commands.
