...
After normal inherited configuration reaches a RunPod remote home, the provider layer writes that home's `config/crew-harness` as `codex` and its `config/crew-dispatch.json` default as Codex with a Claude fallback.
