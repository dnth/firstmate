...
An OMP task additionally records canonical `omp_bin=` and `omp_bun=` launch identities plus the tasktmp-bound `omp_native_socket=` worker-send bridge; [the tmux backend guide](tmux-backend.md#current-behavior-and-safety) owns their recovery and supported-path contract.
