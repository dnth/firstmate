...
- The branch itself: `.omp/extensions/fm-branch-supervision-omp.ts` creates the branch session for the current main session, serializes wakes, mirrors dialog, and merges outcomes.
  ...
  The branch conversation is scoped to exactly one main session. When main performs `/new`, `/resume`, `/fork`, or reload, OMP emits `session_switch`; the primary watcher retires the prior generation and re-arms its replacement before the next model turn, and the branch boundary bumps its own generation so the next wake opens a NEW branch conversation on today's generated prompt rather than reopening an older thread's accumulated memory. The durable outcome store, not that conversation, carries unacknowledged outcomes across the boundary, and the mirror re-anchors so the fresh branch receives the current main session's dialog from its start. `session_shutdown` remains the terminal-process boundary rather than a replacement signal.
  ...
  A settled branch prompt whose last new assistant message carries `stopReason: "error"` is a provider failure that `prompt()` never rejects; two consecutive ones latch the branch off with a merged captain-facing note.
  While latched, main keeps every wake except one bounded recovery probe after each cooldown, which starts at five minutes and doubles to a one-hour ceiling; a probe that ends in a durable report clears the latch with a second note, and a probe that fails reschedules by the current cooldown.
  A settled wake whose prompt produced no durable `fm_branch_report` outcome rejects its settlement too, so a no-op branch turn can never silently claim its granted wake rows.
...
- Branch bash execution: the branch's bash tool is the legacy `createBashToolDefinition` shim wired with a `BashOperations` runner from `.omp/extensions/lib/fm-async-exec.ts`, so every branch command executes through that runner under the spawnHook's injected actor environment - `FM_SUPERVISION_ACTOR=branch`, `FM_LEASE_HOLDER_PID`, and the scriptEnv home overrides - behind the readonly-override prelude.
  That seam is load-bearing since OMP 18.3.0, which moved the bash tool's `env` parameter behind service mode: the shim still forwards a spawnHook `env` into the native bash execute, where it is rejected with "ready and env require a service name." before any process spawns, so routing through `operations.exec` is what keeps the actor injection working on 18.3.x and earlier supported versions alike.
...
A main fall-back follow-up is delivered once OMP accepts it (`sendMessage` resolves); consumption is a separate event that finishes the pending record.
A streaming main consumes at `before_agent_start` or the user `message_start` carrying the exact wake text.
An idle-main injection opens an agent-initiated turn through `agent.prompt()` directly, which emits no `before_agent_start`; there the first `turn_start` after the accepted send is the consumption evidence, and the wake's own `message_start` (role `custom`, `customType` `firstmate-watcher-wake`) acknowledges it too.
Any of these marks the row delivered, and a marker that outlives a full turn boundary without any of them - or whose send-time queue rows were drained - is cleared as consumed at `turn_end`, so a missed consumption signal cannot suppress every later wake.
A follow-up accepted but not yet consumed is kept in the generation's unconsumed set, so a session replacement can replay only the wakes the runtime had not yet started to read.
The episode mirror persisted at `state/extensions/omp-primary-watch/main-fallback-episode.state` records the in-flight marker and the send, consume, and turn-boundary timestamps so a wedge is diagnosable from disk; `bin/fm-guard.sh` warns when that file shows an in-flight wake older than a turn.
...
The branch re-sends any still-undelivered obligation once per generation after a settled wake, batched into one turn; main's consumption events (`before_agent_start`, the first `turn_start` after an idle-main send, or a matching `message_start` with the wake's custom role and type) retire the in-flight marker so a consumed send is not re-sent, while the durable receipt is what actually discharges the obligation.
