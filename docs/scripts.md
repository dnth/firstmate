...
| `fm-spawn.sh`            | Spawn crewmates, scouts, `id=repo` batches, and secondmates, or guardedly recover an ordinary OMP worker or scout |
| `fm-spawn-recovery-lib.sh` | Guarded ordinary OMP recovery identity validation, durable-session selection, staged publication, and rollback support |
