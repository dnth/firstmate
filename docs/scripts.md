...
| `fm-runpod-pod-boot.sh`  | Tracked container boot contract sent to every RunPod pod: provision the remote toolchain on first boot, restore the volume's SSH host key, start sshd, hand readiness to the doctor |
