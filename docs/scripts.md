...
| `fm-turnend-signal.sh`   | Lock-free publisher of the per-generation task turn-end marker state/<id>.turn-ended.<gen>; the consumer (fm-wake-lib.sh) fires only the live gen |
...
| `fm-devin-config.sh`      | Write an isolated mode-600 per-worker Devin config while preserving operator settings |
...
| `fm-inbox-result-lib.sh`  | Own trusted-local note, target, timestamp, artifact, and terminal-result envelope validation |
| `fm-inbox-result.sh`     | Publish immutable trusted-local result envelopes and deliver or retry them through an explicit adapter |
| `fm-inbox-hermes-adapter.sh` | Deliver a validated inbox result through the configured Hermes `send` command |
...
| `fm-stall-recovery.sh`   | Custody-checked bounded auto-recovery before a stale wake: live endpoints escalate untouched, absent endpoints relaunch |
