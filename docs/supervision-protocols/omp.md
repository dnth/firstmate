...
When the branch is unavailable, fallback notifications to MAIN are coalesced to at most one in flight per claimed handling episode, with zero or one successor per turn boundary according to unread MAIN-owned rows; the architecture owner records the contract and bounded attended recovery ([OMP supervision branch](../omp-supervision-branch.md#main-fallback-re-entry-coalescing)).
