...
Known limitation: when the branch is unavailable, current fallback notifications can re-enter MAIN during one claimed, unacknowledged batch; the architecture owner records the coalescing contract and bounded attended recovery ([OMP supervision branch](../omp-supervision-branch.md#main-fallback-re-entry-limitation)).
Use that owner and the [runtime verification record](../verification/runtime-backends.md#omp-main-fallback-re-entry) for the aggregate recovery and evidence; this protocol does not duplicate the defect contract.
