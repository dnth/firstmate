...
Ordinary wake: drain, handle all emitted wakes, reconcile open decisions and unread status lines, record the printed `bin/fm-branch-outcome.sh deliver` receipt for every STATUS OUTCOME BACKSTOP entry after relaying it to the captain, and run the exact `--ack-through` command printed as `WAKE_ACK_REQUIRED`, then repeat that verified wait while supervision is still required.
