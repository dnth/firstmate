...
The irrevocable commit point of a translation is the exit status, because the harness delivers the collected stderr banner only on exit 2, so the owned terminal ledger write is the atomic commit: the winning generation exits 2 unconditionally after it, a refused generation exits 0 silently even after printing, and the once-per-episode failure notice commits in the same owned critical section as the winning failed write.
