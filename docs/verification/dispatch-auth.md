...
`bin/fm-bootstrap.sh` enforces the current quota-axi floor through `bin/fm-quota-axi-lib.sh`; the current-latest policy may place that floor above the schema-introduction version recorded here.
