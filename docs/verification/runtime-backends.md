...
The default OMP launch message is the encoded full brief body (`fm-operational-input.sh encode launch-brief` emits `⁣FIRSTMATE_OP: v1 launch-brief: <body>`). Before delivery, standalone `orchestrate` in an ordinary brief is wrapped in an inline HTML comment so brief prose cannot activate native orchestration accidentally; the `orchestration: enabled` marker deliberately uses the non-matching noun.
