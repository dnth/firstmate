...
A valid trusted lifecycle record decides classification in both directions and the rendered tail is consulted only in its absence, which is why each settled turn above reads `idle hermes-hook` rather than the rendered `idle hermes-tui`, while the pre-record spawn probe still reads `hermes-tui`. That ordering is what keeps a lagging busy redraw from letting `--key C-c` reach an already-idle TUI and exit it.
