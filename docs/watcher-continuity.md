...
For every supported arm path, a successor that observes an accepted down stretch emits `check: rearm-resurface` through the ordinary durable handling path before settling into its live wait or, once the resurface bound trips, resuming full pane supervision.
...
The historical 300-second grace remains the floor for short polls, while watcher and auto-arm paths derive a larger default from `FM_POLL` when a long poll would otherwise outrun that grace; [`turnend-guard.md`](turnend-guard.md#guard-grace-and-the-poll-cadence) owns the formula and reader-specific exceptions.
...
Its real-process fixture also drives an injected arm outage through continuity failure, turn-start healing, and a multi-grace-window supervised turn while asserting the guard never reports `WATCHER DOWN` and no second watcher cycle appears.
...
`tests/fm-omp-primary.test.sh` covers OMP's binding of the same core to its native session, watcher, and shutdown surfaces, pins turn-start re-arm when no arm child is live and its idempotent single-cycle behavior, pins the input-preserving hidden next-turn delivery contract, pins the recovery handling handshake OMP performs before delivering that notification, and pins the single typed wake a refused handshake produces.
