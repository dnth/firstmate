...
That recovery presentation rebuilds a bounded current projection from unacknowledged queue rows, still-unread informational status lines, and the existing cursor-folded OPEN DECISIONS set on every retry, so a still-open decision or buried `note:` answer reappears even when recovery has no queue row of its own and a later exact resolution cannot replay an older rendering.
