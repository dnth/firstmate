...
JSON
  ...
  while [ "$active_screen_wait" -lt 120 ]; do
    ...
    # Wait for the redraw this block actually asserts: Calm persisted on disk, the
    # collapsed-thinking adapter (unconditional, unaffected by the built-in tool gate
    # below) hides, and the retained genuine rows are back on screen. Breaking on any
    # one signal alone can observe a half-redrawn transcript. Built-in tool rows from
    # before this first-ever activation are a separate, documented exception (see
    # fm-calm.ts's file header and docs/calm.md): Pi gives no way to re-point an
    # already-rendered tool row at a definition registered later, so CALM_E2E_OUTPUT
    # and friends stay on screen through this whole redraw rather than disappearing with it.
