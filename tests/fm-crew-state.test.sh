...
# Same coarse-scan guard for an OMP crew (this fork's own harness): unknown
# attribution must reach the OMP crew's ordinary pane fallback, never the older
# failed row, so a live OMP validation is not misreported as a failure.
test_coarse_unresolvable_active_row_omp_crew_uses_pane_fallback() {
  reset_fakes
  local d short; d=$(new_case f10-coarse-guard-omp)
  make_repo_on_branch "$d/wt" fm/feat-f10omp
  short=$(git -C "$d/wt" rev-parse --short=8 HEAD)
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-f10omp.meta" "window=fm:fm-feat-f10omp" "worktree=$d/wt" "kind=ship" "harness=omp"
  FM_FAKE_AXI_STATUS="$(run_running fm/other-crew)"
  FM_FAKE_RUNS_LIST="$(cat <<EOF
  running    fm/feat-f10omp f0f0f0f0  2026-08-27 13:53
  failed     fm/feat-f10omp ${short}  2026-08-27 12:09
EOF
)"
  local out; out=$(run_crew_state "$d" feat-f10omp)
  assert_not_contains "$out" "state: failed" "an OMP crew's unresolvable active row must not fall to the older failed row"
  assert_not_contains "$out" "source: run-step" "unknown attribution must not bind a run for an OMP crew"
  assert_contains "$out" "source: pane" "unknown attribution falls to the OMP crew's pane fallback, not an older row"
  pass "coarse scan guard sends an OMP crew to its pane fallback instead of a superseded failure"
}
