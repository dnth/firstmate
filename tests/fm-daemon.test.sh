...
# The fork-only recovery projection is a second digest path into inject_msg; it
# must obey the same transport bound so an oversized replayed projection cannot
# wedge delivery the same way the escalation buffer did.
test_recovery_projection_digest_is_bounded_and_kept_durable() {
  local dir state fakebin sent raw digest full i item
  dir=$(make_bordered_case digest-recovery-projection)
  state="$dir/state"; fakebin="$dir/fakebin"
  sent="$dir/sent.log"; : > "$sent"
  for i in a b c; do
    item="recovery-$i: "
    while [ "${#item}" -lt 60000 ]; do item+="needs-decision [key=r$i]: still open; "; done
    printf '%s\n' "$item" >> "$state/.subsuper-recovery-escalations"
  done
  printf 'gen-test\n' > "$state/.subsuper-recovery-escalations.generation"
  raw=$(LC_ALL=C wc -c < "$state/.subsuper-recovery-escalations" | tr -d ' ')
  [ "$raw" -gt 131071 ] || fail "fixture projection is only $raw bytes; it must exceed one argument's 131,071-byte ceiling"
  afk_enter "$state"
  LOG="$dir/daemon.log" PATH="$fakebin:$PATH" FM_FAKE_COMPOSER="$dir/composer" FM_FAKE_SENT="$sent" \
    FM_FAKE_SEND_MAX_BYTES=131071 FM_INJECT_CONFIRM_SLEEP=0.05 \
    recovery_projection_flush "$state" gen-test \
    || fail "oversized recovery projection was not delivered: $(cat "$dir/daemon.log" 2>/dev/null)"
  digest=$(grep -F 'Supervisor escalate' "$sent")
  [ "$(printf '%s\n' "$digest" | wc -l | tr -d ' ')" -eq 1 ] || fail "expected exactly one typed digest"
  [ "$(printf '%s' "$digest" | LC_ALL=C wc -c | tr -d ' ')" -le 16384 ] \
    || fail "recovery projection digest is not bounded well below the transport ceilings"
  full=$(printf '%s' "$digest" | sed -n 's/.*full text of every event: \([^ )]*\).*/\1/p')
  [ -n "$full" ] && [ -f "$full" ] || fail "bounded projection digest names no readable full-text file: $digest"
  cmp -s "$full" "$state/.subsuper-recovery-escalations" || fail "full-text file does not hold every projected event verbatim"
  pass "an oversized recovery projection is delivered bounded, with the full text kept durable"
}
