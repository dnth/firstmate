...
cat > "$BASE_RULES" <<'JSON'
{
  "rules": [
    ...
    },
    {
      ...
      "use": [
        ...
        { "harness": "devin", "model": "swe-4.6-medium" },
  ...
  "default": [
    ...
    { "harness": "devin", "model": "swe-4.6-high" }
  ]
}
JSON
cp "$BASE_RULES" "$RULES"

write_quota() {  # <path> <devin spendPriority> [<claude all_models spendPriority>]
  local path=$1 devin=$2 claude=${3:--0.4627}
  cat > "$path" <<JSON
{
  ...
  "providers": [
    ...
    { "provider": "devin", "state": { "status": "fresh" }, "quotaSemantics": { "status": "known", "effectiveAvailability": [
      { "scope": "all_models", "status": "known", "effectivePercentRemaining": 91, "runway": { "status": "through_reset" }, "selection": { "spendPriority": $devin } } ] } },
...
assert_contains "$out" "  profile: --harness 'devin' --model 'swe-4.6-medium'" "argmax picks the highest spendPriority"
...
assert_not_contains "$out" '--effort' "devin profile without effort emits no --effort"
argv=$(cat "$LOG/argv")
...
assert_not_contains "$body" 'swe-model' "use profiles never leave the machine"
...
assert_contains "$out" "  profile: --harness 'devin' --model 'swe-4.6-medium'" "resolution uses the same rules snapshot Jev received"
...
assert_equals 'devin' "$2" "shell-safe profile output preserves the selected harness"
...
printf '%s\n' '{"default":[{"harness":"claude","model":"opus"},{"harness":"devin","model":"swe-4.6-high"}]}' > "$DEFAULT_ONLY"
printf '%s\n' '{"rules":[],"default":[{"harness":"claude","model":"opus"},{"harness":"devin","model":"swe-4.6-high"}]}' > "$EMPTY_RULES"
...
HERMES_RULE="$TMP_ROOT/hermes-rule.json"
printf '%s\n' '{"rules":[{"when":"Hermes work.","use":{"harness":"hermes"}}]}' > "$HERMES_RULE"
cp "$HERMES_RULE" "$RULES"
cat > "$RESPONSE" <<'JSON'
...
reset_log
TYPESAFE_API_KEY=$KEY run code out err "$BRIEF"
assert_contains "$out" 'candidate: hermes:-  provider=codex  scope=all_models  remaining=31%  spendPriority=-0.1649  runway=projected_exhaustion  -> eligible' "hermes uses its resolver-only authoritative quota provider"
...
assert_contains "$out" 'candidate: pi:anthropic/claude-opus-4-8  provider=claude' "the documented Pi default uses its declared Claude provider"
assert_not_contains "$err" 'malformed rules file' "the documented example reaches resolution"
cp "$BASE_RULES" "$RULES"
pass "no-rule fallback, Hermes, OMP explicit provider, and documented configurations resolve"
...
assert_contains "$out" "  profile: --harness 'devin' --model 'swe-4.6-high'" "fall-through resolves among the default profiles"
...
jq '(.providers[] | select(.provider == "devin") | .quotaSemantics.effectiveAvailability[] | select(.scope == "all_models") | .selection.spendPriority) = "high"' "$QUOTA" > "$NONNUMERIC"
write_response "$RESPONSE" rule_4 0.9
TYPESAFE_API_KEY=$KEY QUOTA_AXI_FIXTURE="$NONNUMERIC" run code out err "$BRIEF"
assert_contains "$out" 'candidate: devin:swe-4.6-medium  provider=devin  scope=all_models  remaining=91%  spendPriority=-  runway=through_reset  -> eligible, unranked: spendPriority missing or non-numeric at all_models: not rankable: disclosed uncertainty' "a nonnumeric spendPriority remains eligible but unranked"
...
jq '(.providers[] | select(.provider == "devin") | .quotaSemantics.status) = "partial"' "$QUOTA" > "$PARTIAL"
write_response "$RESPONSE" rule_4 0.9
TYPESAFE_API_KEY=$KEY QUOTA_AXI_FIXTURE="$PARTIAL" run code out err "$BRIEF"
assert_contains "$out" 'candidate: devin:swe-4.6-medium  provider=devin  scope=all_models  remaining=91%  spendPriority=0.7597  runway=through_reset  -> eligible' "a known row from a partial provider remains rankable"
assert_contains "$out" "  profile: --harness 'devin' --model 'swe-4.6-medium'" "partial provider evidence can win the argmax"

PARTIAL_UNKNOWN="$TMP_ROOT/partial-unknown.json"
jq '(.providers[] | select(.provider == "devin") | .quotaSemantics) |= (.status = "partial" | .effectiveAvailability += [
  {"scope":"model:swe-4.6-medium","status":"unknown","runway":{"status":"unknown"}}
])' "$QUOTA" > "$PARTIAL_UNKNOWN"
TYPESAFE_API_KEY=$KEY QUOTA_AXI_FIXTURE="$PARTIAL_UNKNOWN" run code out err "$BRIEF"
assert_contains "$out" 'candidate: devin:swe-4.6-medium  provider=devin  scope=model:swe-4.6-medium  remaining=-%  spendPriority=-  runway=-  bounds=all_models:91%/through_reset,model:swe-4.6-medium:-%/unknown  -> eligible, unranked: quota row model:swe-4.6-medium unknown: not rankable: disclosed uncertainty' "an unknown exact-model row preserves partial known evidence without ranking"
assert_contains "$out" '  note: 2 eligible candidate(s) unranked (devin, kimi)' "clear result lists every provider with unranked uncertainty"
...
assert_contains "$out" 'candidate: devin:swe-4.6-medium  provider=devin  scope=all_models  remaining=0%  spendPriority=-  runway=exhausted_now  bounds=all_models:0%/exhausted_now,model:swe-4.6-medium:-%/unknown  -> not eligible: runway exhausted_now at all_models' "known exhaustion vetoes a candidate despite unknown exact-model evidence"
...
jq '(.providers[] | select(.provider == "devin") | .quotaSemantics) = {
...
assert_contains "$out" 'candidate: devin:swe-4.6-medium  provider=devin  scope=all_models  remaining=-%  spendPriority=-  runway=exhausted_now  -> not eligible: runway exhausted_now at all_models' "unknown provider semantics cannot mask concrete exhaustion"

NO_APPLICABLE="$TMP_ROOT/no-applicable.json"
jq '(.providers[] | select(.provider == "devin") | .quotaSemantics.effectiveAvailability) = [
...
assert_contains "$out" 'candidate: devin:swe-4.6-medium  provider=devin  -> eligible, unranked: no applicable quota row for provider devin: disclosed uncertainty' "a candidate without an applicable row remains eligible but unranked"
assert_contains "$out" '  note: 2 eligible candidate(s) unranked (devin, kimi)' "no-applicable-row uncertainty appears in the clear-result note"
...
assert_contains "$out" "  profile: --harness 'devin' --model 'swe-4.6-high'" "default resolves by argmax"
...
jq '.providers |= map(if .provider == "devin" or .provider == "claude" then .quotaSemantics.effectiveAvailability |= map(.runway.status = "exhausted_now") else . end)' "$QUOTA" > "$NONE"
...
assert_contains "$out" "  profile: --harness 'devin' --model 'swe-4.6-medium'" "quota-axi snapshot drives the argmax"
reset_log
...
for bad in \
  ...
  '{"rules":[{"when":"x","use":{"harness":"claude","provider":""}}]}|each use profile needs harness; model, effort, prewalk_into, and floor must be well formed, and provider must match ^[a-z0-9]+(-[a-z0-9]+)*\z when present' \
  '{"rules":[{"when":"x","use":{"harness":"claude","provider":" claude"}}]}|each use profile needs harness; model, effort, prewalk_into, and floor must be well formed, and provider must match ^[a-z0-9]+(-[a-z0-9]+)*\z when present' \
  '{"rules":[{"when":"x","use":{"harness":"claude","provider":"claude\n"}}]}|each use profile needs harness; model, effort, prewalk_into, and floor must be well formed, and provider must match ^[a-z0-9]+(-[a-z0-9]+)*\z when present' \
  '{"rules":[{"when":"x","use":{"harness":"codex","floor":{"scope":"all_models","min_percent":20,"provider":"claude"}}}]}|each use profile needs harness; model, effort, prewalk_into, and floor must be well formed, and provider must match ^[a-z0-9]+(-[a-z0-9]+)*\z when present' \
  ...
  '{"rules":[{"when":"x","use":{"harness":"grok","effort":"max"}}]}|each use profile effort must be supported by its harness' \
  ...
  '{"rules":[{"when":"x","use":{"harness":"pi","model":"anthropic/claude-sonnet-4-5"}}]}|use profiles whose harness lacks one authoritative provider family require provider: pi' \
  '{"rules":[{"when":"x","use":{"harness":"codex"}}],"default":{"harness":"pi","model":"anthropic/claude-opus-4-8"}}|default profiles whose harness lacks one authoritative provider family require provider: pi'; do
  printf '%s\n' "${bad%%|*}" > "$RULES"
  TYPESAFE_API_KEY=$KEY run code out err "$BRIEF"
