#!/usr/bin/env bash
# Live Herdr submit-confirmation guard (opt-in live family).
#
...
# either signal, and a stub cannot prove the pre-Enter payload proof reads a
# real Claude composer correctly. This guard launches real Claude Code in an
# isolated Herdr lab and requires fm_backend_herdr_send_text_submit to report
# empty for a landed idle steer. It fails naming the harness and version
...
if [ "${FM_HERDR_SUBMIT_CONFIRM_LIVE:-0}" != 1 ]; then
  echo "skip: set FM_HERDR_SUBMIT_CONFIRM_LIVE=1 for the live Herdr submit-confirmation guard"
  exit 0
fi
...
# Claude registers the agent idle before it draws the framed composer; the
# pre-send proof correctly refuses while no composer row is readable. Wait
# for the bare `❯` row to render before exercising the submit.
i=0
while [ "$i" -lt 45 ]; do
  case "$(lab pane read "$PANE" --source recent --lines 40 2>/dev/null || true)" in
    *'❯ '*) break ;;
    *❯*) break ;;
  esac
  i=$((i + 1))
...
[ "$i" -lt 45 ] || fail "Claude Code ($VERSION) on $HERDR_VER reported idle but never drew its composer row"

TOKEN="FMHERDRPONG$$_$RANDOM"
# 'unknown' is the conservative unproven verdict: while Claude is mid-turn the
# composer row is not drawn, so a native-idle poll can fall through to a
# composer read that has nothing to classify. That race predates the payload
# proof and is inherent to the live pane, so retry the whole submit once on a
# bare 'unknown' - the proof makes re-submission safe (a leftover draft is
