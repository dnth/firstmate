...
test_kimi_launch_then_send_is_verified() {
  ...
  [ "$launch" = "FM_TASK_ID='$id' FM_ROOT_OVERRIDE= FM_STATE_OVERRIDE= FM_DATA_OVERRIDE= FM_PROJECTS_OVERRIDE= FM_CONFIG_OVERRIDE= '$FAKEBIN_DIR/kimi' --model 'kimi-code/k3' --auto" ] \
    || fail "kimi launch did not use the worker identity, absolute binary, model, and --auto only: $launch"
...
test_kimi_hook_is_silent_and_requires_registered_workspace_token() {
  local id rec out rc hook target target_gen gen token registry registry_content no_token snapshot_before snapshot_after fakebin
  ...
  # The hook resolves the registry spawn_gen and publishes state/<id>.turn-ended.<spawn_gen>.
  gen=$(sed -n 's/^spawn_gen=//p' "$registry")
  target_gen="$HOME_DIR/state/$id.turn-ended.$gen"
  ...
  assert_absent "$target_gen" "tokenless Kimi hook invocation touched a task marker"
  ...
  assert_present "$target_gen" "registered Kimi hook invocation did not publish the per-generation marker"
  ...
  assert_absent "$target_gen" "a legacy one-line Kimi registry entry remained active"
  printf '%s\n' "$registry_content" > "$registry"

  # The publisher is unconditional (incarnation gating is the consumer's job), so a
  # registered hook publishes a stamped marker even with the meta gone; the consumer
  # discards it against the missing incarnation (covered in fm-wake-queue.test.sh).
  rm -f -- "$target_gen" "$HOME_DIR/state/$id.meta"
  ...
  assert_present "$target_gen" "a registered Kimi hook must publish a per-generation marker even with the meta gone"
  ...
  assert_absent "$target_gen" "Kimi hook without jq touched the turn-end marker"
...
test_kimi_falls_back_to_expanded_home_binary() {
  ...
  launch=$(cat "$CASE_DIR/launch.log")
  [ "$launch" = "FM_TASK_ID='$id' FM_ROOT_OVERRIDE= FM_STATE_OVERRIDE= FM_DATA_OVERRIDE= FM_PROJECTS_OVERRIDE= FM_CONFIG_OVERRIDE= '$fallback' --auto" ] \
