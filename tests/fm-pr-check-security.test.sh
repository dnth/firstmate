...
# private atomic artifacts, and teardown cleanup.
set -u
...
test_live_artifact_single_link_and_privacy_validation() {
  ...
  set +e
  FM_HOME="$dir/home" "$REGISTER" custom >/dev/null 2>/dev/null
  rc=$?
  set -e
  ...
  pass "live poll and custom-check boundaries require single-link private files"
}
...
test_teardown_removes_poll_artifacts() {
  ...
  fm_pr_poll_merge_mark_notified "$dir/home/state" task-a github github.com o/r 1 \
...
SH
  ...
  [ ! -e "$dir/home/state/task-a.pr-poll-merge-notified" ] \
    || fail "teardown left the merge-notified marker"
  ...
  [ ! -e "$dir/home/state/task-a.pr-poll-registration" ] \
    || fail "teardown left the PR poll registration"
  [ ! -e "$dir/home/state/task-a.check-trust" ] \
    || fail "teardown left the custom check registration"
  pass "teardown removes live PR-check artifacts"
}
...
test_merged_poll_reregistration_after_notification_is_absorbed() {
  ...
  ! grep -F 'merged-task-a' "$state/.wake-queue" >/dev/null 2>&1 \
...
# The captain merging a PR on the forge is the same outcome as a merge this
# home performed: bin/fm-merge-outcome-lib.sh carries both to the parent on
