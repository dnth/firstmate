...
for protected in "$ROOT/bin/fm-runpod.sh" "$ROOT/bin/fm-x-reply.sh" \
  ...
  if when "$H" arm "protected-$policy_case" --stable 1 \
    --condition true --action "$protected" status 2>"$TMP_ROOT/protected-$policy_case.err"; then
    fail "a protected lifecycle or public-reply entrypoint must not be armed"
  fi
  assert_grep "not allowlisted" "$TMP_ROOT/protected-$policy_case.err" \
    "the protected entrypoint refusal names the allowlist"
  assert_absent "$H/state/procevent/when-protected-$policy_case.source" \
    "a protected entrypoint leaves no registration"
done
if when "$H" arm protected-condition --stable 1 \
  --condition "$ROOT/bin/fm-x-poll.sh" --action "$ACT" "$TMP_ROOT/policy-act" \
  2>"$TMP_ROOT/protected-condition.err"; then
  fail "a protected X-mode entrypoint must not be armed as a condition"
fi
...
CONDITION_TAMPER_LOG="$TMP_ROOT/condition-tamper-effect"
CONDITION_TAMPER_ACT="$TMP_ROOT/condition-tamper-act"
...
when "$H" arm condition-tamper --stable 1 \
  --condition "$MUTABLE_COND" \
  --action "$ACT" "$CONDITION_TAMPER_ACT" >/dev/null
cat > "$MUTABLE_COND" <<SH
#!/usr/bin/env bash
printf 'mutated condition ran\n' >> '$CONDITION_TAMPER_LOG'
exit 0
SH
chmod +x "$MUTABLE_COND"
pe "$H" reconcile >/dev/null
...
assert_grep 'condition executable bytes' "$RESULT" "the refusal names the condition trust binding"
assert_absent "$CONDITION_TAMPER_LOG" "the mutated condition was not executed"
assert_absent "$CONDITION_TAMPER_ACT" "the action was not executed after condition tampering"
assert_absent "$H/state/when/when-condition-tamper.fired" "no fire was claimed after condition tampering"
