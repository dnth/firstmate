...
# starts the worker from the fetched origin/default tip or stops safely.
set -u
...
test_stale_pool_base_refreshes_before_branching() {
  ...
  assert_grep 'must survive a newly spawned branch' "$POOL_DIR/advanced-main.txt" \
    "the refreshed pool omitted advanced-main content"
...
test_diverged_pool_refuses_without_discarding_commits() {
  ...
  rec=$(make_case diverged-refusal "$id")
  read_case_record "$rec"
  printf 'keep this committed work\n' > "$POOL_DIR/local.txt"
  git -C "$POOL_DIR" add local.txt
  git -C "$POOL_DIR" -c user.name='Firstmate Tests' -c user.email='tests@example.invalid' commit -qm local-pool-work
  before=$(git -C "$POOL_DIR" rev-parse HEAD)

  out=$(run_spawn "$id" --mode no-mistakes --yolo off)
  status=$?
  [ "$status" -ne 0 ] || fail "spawn succeeded despite unique pooled commits"
  assert_contains "$out" "not fast-forwardable" \
    "spawn did not clearly refuse a non-fast-forwardable pooled worktree"
  [ "$(git -C "$POOL_DIR" rev-parse HEAD)" = "$before" ] \
    || fail "spawn moved HEAD while refusing unique pooled commits"
  assert_grep 'keep this committed work' "$POOL_DIR/local.txt" \
    "spawn discarded committed pooled work while refusing the pool"
...
# leaves no claim naming a task with no record. The make_case pool is already a
# managed <pool>/<slot>/<repo> layout, so the claim lands beside the checkout.
test_pool_slot_claim_follows_the_spawn_outcome() {
  local rec id out status before slot_claim
  ...
  assert_grep "worktree=$POOL_DIR" "$HOME_DIR/state/$id.meta" \
    "spawn did not publish the pool slot as its worktree"
  ...
  rec=$(make_case slot-claim-aborted "$id")
  read_case_record "$rec"
  slot_claim="$(dirname "$POOL_DIR")/.fm-slot-owner"
  git -C "$POOL_DIR" remote set-url origin "file://$CASE_DIR/missing-origin.git"
  out=$(run_spawn "$id" --mode no-mistakes --yolo off)
