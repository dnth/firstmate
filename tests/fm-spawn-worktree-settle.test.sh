...
test_ship_relaunch_refuses_raw_worktree_command() {
  local rec id out status
  id=ship-relaunch-raw-z5
  rec=$(make_settle_case ship-relaunch-raw "$id" 0)
  read_settle_record "$rec"
  mkdir -p "$HOME_DIR/data/$id"
  printf 'Delivery contract: mode=no-mistakes\nrefuse raw relaunch\n' > "$HOME_DIR/data/$id/brief.md"
  {
  ...
  out=$(FM_HOME="$HOME_DIR" FM_STATE_OVERRIDE="$HOME_DIR/state" \
    ...
    --harness './worker --resume' --relaunch --mode no-mistakes --yolo off 2>&1)
  status=$?
  set -e
  [ "$status" -ne 0 ] || fail "raw ship relaunch unexpectedly succeeded"
  assert_contains "$out" "raw relaunch commands that require a worktree are refused" \
    "raw ship relaunch did not explain the refusal"
  assert_grep "worktree=$WT_DIR" "$HOME_DIR/state/$id.meta" \
    "raw ship relaunch changed the recorded worktree"
