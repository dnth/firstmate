...
test_spawn_tmux_window_construction() {
  ...
  # Bug 2 fix (b): guarded treehouse-get and the worktree wait loop target the stable id.
  assert_grep "send-keys -t @spawnwid FM_TREEHOUSE_REJECT_SECONDMATE_MARKERS=1 $ROOT/bin/fm-treehouse-get.sh Enter" "$rec" \
    "guarded treehouse get must be sent to the stable window id"
