...
# nothing is typed and the distinct return code (6 on this fork, where 3-5 are
# OMP-native and Hermes-lock outcomes) lets callers route to recovery. An
# unreadable endpoint still rings, so a blind classifier never starves a live
# worker.
test_ring_skips_dead_agent() {
  ...
  PATH="$dir/fakebin:$PATH" FM_SEND_LOG="$log" FM_FAKE_TMUX_AGENT=zsh \
    inbox_lib "$state" fm_task_inbox_ring tmux sess:fm-t1 "$rec" fm-t1 || rc=$?
  [ "$rc" = 6 ] || fail "a dead agent should return 6 from the ring, got $rc"
  ...
  PATH="$dir/fakebin:$PATH" FM_SEND_LOG="$log" FM_FAKE_TMUX_MISSING=1 \
    inbox_lib "$state" fm_task_inbox_ring tmux sess:fm-t1 "$rec" fm-t1 || rc=$?
  [ "$rc" = 6 ] || fail "a missing endpoint should return 6 from the ring, got $rc"
