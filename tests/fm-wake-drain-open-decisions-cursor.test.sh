...
test_read_failure_preserves_state_for_retry() {
  ...
  if FM_STATE_OVERRIDE="$state" FM_STATUS_SPAN_READER="$reader" "$DRAIN" > "$out" 2>/dev/null; then
    fail "wake drain did not fail closed after the injected presentation read failure"
  fi
  [ ! -s "$out" ] \
  ...
  pass "a failed presentation read fails closed and preserves status state for retry"
}
...
test_legacy_unversioned_cursor_refolds_corr_tagged_decision() {
  ...
  FM_STATE_OVERRIDE="$state" FM_OPEN_DECISIONS_READ_PROBE="$probe" "$DRAIN" > "$out" \
    || fail "drain failed while migrating the legacy unversioned cursor"
  grep -F 'task7 [key=loan-installment-cadence-amount] needs-decision: pick the cadence' "$out" >/dev/null \
    || fail "the legacy cursor hid the corr-tagged decision after migration: $(command cat "$out")"
  probe_bytes=$(last_probe_bytes "$probe" "$status")
  [ "$probe_bytes" = "$status_bytes" ] \
    || fail "the legacy cursor read $probe_bytes bytes instead of refolding all $status_bytes authoritative bytes"

  pass "a legacy unversioned cursor is rebuilt so a corr-tagged decision surfaces"
}
