...
test_pending_reply_resolution_surfaces_once() {
  ...
  {
    printf 'resolved [key=pending-reply-abcdef0123456789]: correlated pending reply reconciled\n'
    printf 'note: re-read acknowledgement\n'
  } >> "$status"

  FM_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed on a pending-reply resolution"

  grep -F 'resolved [key=pending-reply-abcdef0123456789]: correlated pending reply reconciled' "$out" >/dev/null \
  ...
  if grep -F 'correlated pending reply reconciled' "$out" >/dev/null; then
