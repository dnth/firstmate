...
# Must match bin/fm-watch.sh's stat_sig exactly. This fork's signal scan compares
# an inode:size:nanosecond-mtime signature so a same-second recreate cannot slip
# through; priming a coarser signature would never suppress, and the fixture
# watcher would race an unrelated signal wake into the case under test.
...
test_rearm_resurfaces_durable_queue_and_remote_open_decision() {
  ...
  # bin/fm-procevent-remote-reply.sh accepts one bracket token before the colon
  # and requires a correlation, so a keyed decision carries its corr in the note.
  write_remote_delta "$result" \
    'needs-decision [key=remote-signoff]: remote secondmate is held for captain sign-off (corr=0123456789abcdef)'
  ...
  grep -F 'remote secondmate is held for captain sign-off (corr=0123456789abcdef)' "$dir/baseline-drain.out" >/dev/null \
  ...
  grep -F 'ios [key=remote-signoff] needs-decision: remote secondmate is held for captain sign-off (corr=0123456789abcdef)' "$drainout" >/dev/null \
  ...
  grep -F 'ios [key=remote-signoff] needs-decision: remote secondmate is held for captain sign-off (corr=0123456789abcdef)' \
