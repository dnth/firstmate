...
# Wait until the watcher has completed a whole poll cycle, or exited first.
# The beacon is touched at the top of each poll, so observing two distinct
# mtimes proves the cycle between them completed instead of passing vacuously
# while a loaded machine is still doing bounded watcher startup work.
...
test_watcher_pause_default_is_2700_and_override_wins() {
  ...
  ack_stopped_cycle "$state" || fail "could not acknowledge the default-cadence pause recheck"
...
test_exited_declared_pause_is_bounded_but_live_gate_surfaces() {
  local dir state fakebin out capture_file statusf window key pane_hash sig pid back round wakes bare rc
  ...
  while [ "$round" -le 6 ]; do
    ...
    if wait_for_exit "$pid" 30; then
      ...
      rc=$?
      [ "$rc" -eq 124 ] || fail "dead-agent watcher round $round failed"
    fi
    round=$((round + 1))
  done
...
test_absorbed_replacement_wait_does_not_inherit_the_old_throttle() {
  ...
  for spec in \
    ...
    'captain-held-replacement|captain-held [key=route]: awaiting the routing call|captain-held [key=release]: awaiting the release call|awaiting external'
  do
    name=${spec%%|*}; spec=${spec#*|}
...
SH
  ...
  reap "$pid"
  ack_stopped_cycle "$state" || fail "could not acknowledge the fresh remote beacon stop"
  ...
  reap "$pid"
  ack_stopped_cycle "$state" || fail "could not acknowledge the hung remote beacon stop"
...
SH
  ...
  ack_stopped_cycle "$state" || fail "could not acknowledge the $mode fallback probe stop"
  printf '%s\n' $(( $(date +%s) - 500 )) > "$state/.stale-since-$key"
  : > "$out"
  PATH="$runtimebin" FM_FAKE_TMUX_WINDOW="$window" \
    FM_STATE_OVERRIDE="$state" FM_CREW_STATE_BIN="$fakebin/fm-crew-state.sh" FM_ON_BIN="$on_bin" \
...
test_secondmate_stale_supervisor_beacon_escalates() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  # The synthetic stale-secondmate hash intentionally differs from the seeded
  # pane hash, so production needs three one-second polls before publishing the
  # wedge timer. Leave scheduling headroom beyond that exact boundary.
  ...
  reap "$pid"
  ack_stopped_cycle "$state" || fail "could not acknowledge the stale secondmate beacon stop"
...
SH
  ...
  reap "$pid"
  ack_stopped_cycle "$state" || fail "could not acknowledge the secondmate capture-failure stop"
...
SH
  ...
  reap "$pid"
  ack_stopped_cycle "$state" || fail "could not acknowledge the changing secondmate capture stop"
...
# A single blocking external wait can keep a declared-pause pane semantically
# busy past the completed-turn bound, bypassing the ordinary idle pause path.
# Pin both sides of the discriminator on the same pane: the declared pause uses
# the long cadence, and lifting the declaration restores wedge escalation.
test_busy_declared_pause_uses_long_cadence_then_restores_wedge() {
  ...
  reap "$pid"
  ack_stopped_cycle "$state" || fail "could not acknowledge the intentional busy declared-pause stop"
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 40 || { reap "$pid"; fail "a declared pause past the long cadence was never rechecked"; }
  ...
  [ ! -e "$state/.paused-$key" ] || fail "a lifted pause left declared-pause bookkeeping behind"
  reap "$pid"
  ack_stopped_cycle "$state" || fail "could not acknowledge the intentional lifted-pause stop"
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 40 || { reap "$pid"; fail "a lifted pause on an over-age busy pane no longer wedge-escalates"; }
  grep -F "possible wedge" "$out" >/dev/null || fail "the restored busy-turn escalation did not flag a possible wedge: $(cat "$out")"
  pass "a busy declared pause uses the long cadence, and lifting it restores wedge escalation"
}
...
# --- completion-boundary probe: idle-with-open-work --------------------------
# The completion-reporting contract (generated by bin/fm-brief.sh) requires a
# terminal or paused status within FM_IDLE_OPEN_WORK_SECS of turn-end while the
# task's board row is in flight. bin/fm-watch.sh enforces it with a
# time-anchored probe that runs BEFORE the pane-hash comparison, so a ticking
# idle pane - whose hash changes every poll and can never enter the stable-hash
# stale path - cannot dodge it. That hash gap is exactly how silent panes
# survived for hours in the field.

# Rewrite <file> with fresh content on a short cadence until killed: the
# ticking-pane fixture. Its hash never repeats, so the stable-hash stale path
# can never engage; any exit below can only come from the probe under test.
tick_pane() {  # <file>
  local f=$1 i=0
  while :; do
    i=$((i + 1))
    printf 'ticking clock %s\n' "$i" > "$f"
    sleep 0.2
  done
}

# write_backlog <data-dir> <in-flight-task|-> [queued-task]: the minimal board
# board_row_in_flight scans - headings plus one checkbox row per task.
write_backlog() {
  local data=$1 inflight=$2 queued=${3:-}
  mkdir -p "$data"
  {
    printf '## In flight\n\n'
    [ "$inflight" = - ] || printf -- '- [ ] %s fixture row\n\n' "$inflight"
    printf '## Queued\n\n'
    [ -z "$queued" ] || printf -- '- [ ] %s fixture row\n\n' "$queued"
    printf '## Done\n'
  } > "$data/backlog.md"
}
...
test_idle_open_work_ticking_pane_escalates() {
  ...
  statusf="$state/tick.status"
  printf 'working: mid-implementation\n' > "$statusf"
  write_backlog "$data" tick
  # The last status is nonterminal and far past the bound, and with no
  # live-generation turn-end marker the spawn record anchors the bound - age it
  # too, then prime .seen-* so the signal scan does not pre-empt the probe.
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    FM_FAKE_CREW_STATE='state: unknown · source: none · idle pane' \
  ...
  if ! wait_for_exit "$pid" 100; then
    stop_pid "$tickpid"
    fail "a ticking idle pane with an in-flight row and a stale nonterminal status did not escalate"
  fi
  stop_pid "$tickpid"
  ...
  pass "a ticking idle pane with in-flight open work escalates as idle-with-open-work"
}
...
test_idle_open_work_queued_row_is_not_probed() {
  ...
  printf 'working: mid-implementation\n' > "$statusf"
  # The row sits under Queued, not In flight: a queued task owes no boundary
  # report, so the probe must not even record its marker.
  write_backlog "$data" - queued
  set_mtime $(( $(date +%s) - 3600 )) "$statusf"
  ...
  if ! wait_poll_cycle "$state" "$pid"; then
    stop_pid "$tickpid"
    fail "a queued row was probed as idle-with-open-work (should be inert): $(cat "$out")"
  fi
...
test_idle_open_work_recent_status_is_not_probed() {
  ...
  printf 'ticking clock 0\n' > "$capture_file"
  printf 'window=%s\nkind=ship\nmode=direct-PR\n' "$window" > "$state/recent.meta"
  statusf="$state/recent.status"
  # A working: line that landed seconds ago is inside the bound: the worker is
  # still within its reporting window, so no probe and no marker.
  printf 'working: mid-implementation\n' > "$statusf"
...
test_idle_open_work_paused_and_terminal_skip_the_probe() {
  ...
  for pair in "held:paused: awaiting the upstream release" "finished:done: PR https://github.com/example/repo/pull/3"; do
    ...
    printf '%s\n' "$lastline" > "$statusf"
    write_backlog "$data" "$id"
    # Even with every time gate past the bound, a declared pause owns its own
    # cadence and a terminal line already discharged the contract - neither is
    # worker silence, so the probe must not fire or mark.
    set_mtime $(( $(date +%s) - 3600 )) "$statusf"
...
test_idle_open_work_provably_working_is_absorbed() {
  ...
  printf 'ticking clock 0\n' > "$capture_file"
  printf 'window=%s\nkind=ship\nmode=no-mistakes\n' "$window" > "$state/pipeline.meta"
  statusf="$state/pipeline.status"
  printf 'working: handed to no-mistakes\n' > "$statusf"
  write_backlog "$data" pipeline
  set_mtime $(( $(date +%s) - 3600 )) "$statusf"
  ...
  if ! wait_poll_cycle "$state" "$pid"; then
    ...
    fail "a provably-working crew was escalated as idle-with-open-work: $(cat "$out")"
  fi
  stop_pid "$tickpid"; reap "$pid"
  ...
  # The marker rate-limits the whole probe - including the crew-state read - to
  # once per bound, so a second run inside it absorbs silently with no new
  # triage line.
  : > "$out"; : > "$state/.watch-triage.log"
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    FM_STATE_OVERRIDE="$state" FM_DATA_OVERRIDE="$data" FM_CREW_STATE_BIN="$fakebin/fm-crew-state.sh" \
    FM_IDLE_OPEN_WORK_SECS=60 FM_POLL=1 FM_SIGNAL_GRACE=1 FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 \
    "$WATCH" > "$out" &
  ...
  if ! wait_poll_cycle "$state" "$pid"; then
    unset FM_FAKE_CREW_STATE
    fail "the rate-limited probe re-escalated inside the bound: $(cat "$out")"
  fi
  reap "$pid"
  unset FM_FAKE_CREW_STATE
  [ ! -s "$state/.watch-triage.log" ] || fail "the rate-limited probe re-ran the crew-state read inside the bound"
